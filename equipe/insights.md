...
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **205** — ativos: **154** (bounces cache: 47). Fila de prospecção: **165 empresas / 154 contatados / 32 bounce / 0 pendentes**.

### Números do estado real (pós-tick 13:31)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 47).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 47 do cache já tentados).
- **Watchdog:** ✅ **exit 0 de primeira** — saudável, sem follow-ups atrasados, bounces sem correção ou leads parados.
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
...
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`) — caixa limpa.
- **enviar_lote --status:** ✅ 165 empresas / 154 contatados / 32 bounce / **0 pendentes**.
- **Dashboard:** **179 apresentações / 152 entregues / 27 bounces (15%) / 14 respostas / taxa 7,8%** (meta >3%) — acima da meta pelo 2º mês. 135 aguardando resposta/follow-up · 25 inbound acumulados.

### 🆕 Novidades na caixa
- **Nenhuma resposta nova** desde a Andorinha (12/09 12:02, recusa cordial — já atendida). Caixa limpa, zero respostas órfãs, zero pendências. **Sem pedidos de relatório ESG/certificado** — sem geração de PDF neste tick.

### Problemas encontrados e correções
- **Nenhum neste tick** (watchdog exit 0). Pendências estruturais mantidas: fila em **0 pendentes** (reposição do Prospector segue sendo o gargalo nº1) e Netlify sem créditos (403 — GitHub Pages único no ar). Toques WhatsApp humanos vencidos: Savegnago (10/09), Grupo IMC (10/09), Selmi (11/09).
...
3. **Grupo IMC / Frango Assado — toque WhatsApp com prazo 10/09 VENCIDO.**
4. **Selmi (compras@selmi.com.br) — toque WhatsApp com prazo 11/09 VENCIDO.**
5. **FP1/FP2 de supermercados em monitoramento (nicho que mais converte):** Dalben, Amigão, Tenda Atacado, Paulistão (Grupo Savegnago), Enxuto, Tauste, Chimar — monitorar retornos dos canais comerciais.
6. **Andorinha Hiper Center (SALTO)** — loja na cidade-base; se responder, priorizar coleta-teste na unidade de Salto.

### Nota
Tick de rotina totalmente limpo: watchdog exit 0 no 1º run, 0 follow-ups atrasados, caixa sem respostas pendentes, 0 pedidos de ESG. Nenhuma correção necessária. Próximas ações humanas seguem: toques de WhatsApp vencidos (Savegnago, Grupo IMC, Selmi) e coleta-teste com Queijos Itupeva.

## 2026-09-14 (segunda) — ATENDENTE (tick 13:01)

### ✅ Tick limpo — watchdog exit 0 de primeira (15º tick verde consecutivo)
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **205** — ativos: **154** (bounces cache: 47). Fila de prospecção: **165 empresas / 154 contatados / 32 bounce / 0 pendentes**.
...
### Problemas encontrados e correções
- **Nenhum neste tick** (watchdog exit 0). Pendências estruturais mantidas: fila em **0 pendentes** (a rodada de +19 do Prospector às 10:31 já foi consumida no mesmo lote — reposição de 5-8/dia segue sendo o gargalo nº1) e Netlify sem créditos (403, GitHub Pages único no ar).

### Leads quentes (destaque — ação humana)
1. **🔥 QUEIJOS ITUPEVA (NEGOCIAÇÃO ATIVA desde 16:30 11/09):** Rafael Galvão pediu negociação no WhatsApp (11 940333759) → **AÇÃO HUMANA: chamar** (nicho vencidos >40% gordura; Frescal 200 kg/mês + manteiga/requeijão/doce 50–100 kg/mês; referência R$0,50–1,50/kg conforme dificuldade de separação; **descaracterização é cortesia, não cobrada**).
...
4. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **PRAZO 11/09 VENCIDO → toque leve se silêncio.**
5. **Novos de alto valor a monitorar (apresentação enviada ontem/hoje):** **Granol** (óleos vegetais — nicho 1) e **Eurofarma** (farmacêutica), além de Gerresheimer e Mococa — se responderem, tratar como lead quente prioritário.
6. **FP2 no ar (nicho vencidos):** Sorocaba Frios, Megasabor, Grupo Lanchero — 2º toque da sequência.
7. **Em aberto (sem novidade):** Dalben, Paulistão, Amigão, Enxuto, Tauste, Chimar (FP1); Tenda Atacado (FP2); hotéis V6 (Fazenda Capoava, Itu Garden, Brisa Itu, Thermas São Pedro, Dona Carolina); Sumerbol (Ivone), GoodBom (Laura), Rede Boa (#23915), Arcor/Bagley, Sanofi (02995121); Confiança (stand by); Andorinha (recusa cordial, recontato futuro).

### Nota
Tick totalmente limpo: watchdog exit 0 de primeira (15º verde), caixa sem respostas pendentes (0 novas), fila sem pendentes e sem reposição nova desde as 10:31. Funil saudável (7,8%, acima da meta de 3%). Sem pedidos de relatório ESG (nenhum PDF gerado). Ações humanas seguem em aberto: **chamar Rafael (Queijos Itupeva) e toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09)** — são elas que transformam o funil de 7,8% em coleta-teste/contrato.

---

## 2026-09-14 (segunda) — ATENDENTE (tick 12:01)

### ✅ Tick limpo — watchdog exit 0 de primeira (14º tick verde consecutivo)
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **205** — ativos: **154** (bounces cache: 47). Fila de prospecção: **165 empresas / 154 contatados / 32 bounce / 0 pendentes**.

### Números do estado real (pós-tick 12:01)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 47).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 47 do cache já tentados).
- **Watchdog:** ✅ **exit 0 de primeira** — saudável, sem follow-ups atrasados, bounces sem correção ou leads parados.
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
...
### Nota
Tick totalmente limpo: watchdog exit 0 de primeira (14º verde), caixa sem respostas pendentes (0 novas), fila sem pendentes e sem reposição nova desde as 10:31. Funil saudável (7,8%, acima da meta de 3%). Sem pedidos de relatório ESG (nenhum PDF gerado). Ações humanas seguem em aberto: **chamar Rafael (Queijos Itupeva) e toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09)** — são elas que transformam o funil de 7,8% em coleta-teste/contrato.

---

## 2026-09-14 (segunda) — ATENDENTE (tick 11:01)

### ✅ Watchdog exit 0 de primeira — 13º tick verde consecutivo
- **Estado**: watchdog **exit 0** — nenhum follow-up atrasado, nenhum bounce sem correção, nenhum lead parado. Credencial OK (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.**
- **Leads totais**: **205** — ativos: **154** (bounces cache: 47). **Fila de prospecção: 165 empresas / 154 contatados / 32 bounce / 0 pendentes**.

### Números do estado real (pós-tick 11:01)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 47).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 47 do cache já tentados).
- **Watchdog:** ✅ exit 0 — saudável, sem atrasos.
...
### Problemas encontrados e correções
- **Nenhum neste tick** (watchdog exit 0). Pendências estruturais mantidas: fila em **0 pendentes** (a rodada forte de +19 do Prospector às 10:31 já foi consumida no mesmo lote — reposição de 5-8/dia segue sendo o gargalo nº1) e Netlify sem créditos (403, GitHub Pages único no ar).
...
3. **Frango Assado / Grupo IMC — #2:** **PRAZO 10/09 VENCIDO → toque no WhatsApp (11) 96785-9631.**
4. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **PRAZO 11/09 VENCIDO → toque leve se silêncio.**
5. **Novos de alto valor a monitorar (apresentação enviada hoje):** **Granol** (óleos vegetais — nicho 1) e **Eurofarma** (farmacêutica) — se responderem, tratar como lead quente prioritário.
6. **FP2 no ar hoje (nicho vencidos):** Sorocaba Frios, Megasabor, Grupo Lanchero — 2º toque da sequência.
7. **Em aberto (sem novidade):** Dalben, Paulistão, Amigão, Enxuto, Tauste, Chimar (FP1); Tenda Atacado (FP2); hotéis V6 (Fazenda Capoava, Itu Garden, Brisa Itu, Thermas São Pedro, Dona Carolina); Sumerbol (Ivone), GoodBom (Laura), Rede Boa (#23915), Arcor/Bagley, Sanofi (02995121); Confiança (stand by); Andorinha (recusa cordial, recontato futuro).

### Nota
Tick totalmente limpo: watchdog exit 0 de primeira (13º verde), caixa sem respostas pendentes (0 novas), fila sem pendentes e sem reposição nova desde as 10:31. Funil saudável (7,8%, acima da meta de 3%). Sem pedidos de relatório ESG (nenhum PDF gerado). Ações humanas seguem em aberto: **chamar Rafael (Queijos Itupeva) e toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09)** — são elas que transformam o funil de 7,8% em coleta-teste/contrato.

---

## 2026-09-14 (segunda) — ATENDENTE (tick 10:31)

### ⚠️ Watchdog exit 1 resolvido — 3 FP2 disparados (nicho gorduras/laticínios)
- **Estado**: watchdog **exit 1** com 1 problema: **3 follow-ups FP2 atrasados** (Sorocaba Frios, Megasabor/Mega Ingredientes, Grupo Lanchero — todos do nicho RESÍDUOS VENCIDOS >40% gordura). Credencial OK (`SMTP OK · IMAP OK`). **Resolvido no mesmo tick** → watchdog re-executado: **exit 0** ✅
- **Leads totais**: **205** — ativos: **154** (bounces cache: 47). **Fila de prospecção: 165 empresas / 154 contatados / 32 bounce / 0 pendentes**.

### Números do estado real (pós-tick 10:31)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 47).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 47 do cache já tentados).
- **Watchdog:** ⚠️ exit 1 (3 FP2 atrasados) → **RESOLVIDO**: `prospecao_followup.py` disparou 3 FP2 (10:31): **Sorocaba Frios, Megasabor, Grupo Lanchero** — 3/3 OK, sem bounce imediato.
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`) — caixa limpa.
- **enviar_lote --status:** ✅ 165 empresas / 154 contatados / 32 bounce / **0 pendentes**.
- **Dashboard:** **179 apresentações / 152 entregues / 27 bounces (15%) / 14 respostas / taxa 7,8%** (meta >3%) — acima da meta pelo 2º mês. Taxa recalculou de 8,8% → 7,8% porque o denominador subiu (+19 apresentações novas hoje, nenhuma resposta ainda).

### 🆕 19 leads novos hoje (Prospector, ids 187–205) — rodada forte do nicho gorduras
- **13 ativos**: Gerresheimer Indaiatuba (embalagens p/ medicinais), Eurofarma (farmacêutica), Granol (óleos vegetais), Mococa (laticínios), Alma Quente (molhos artesanais), Laticínios Aviacao, Sítio Piscke (Elias Fausto), Bom Rech (Itu), EmbaVi (Valinhos), Trani (Itapira), Homemade (Itupeva), Leiteria Prosperidade, Queijos Rosa.
- **3 encerrados**: Mars Brasil, Nestlé (Araras), Vitafor.
- **3 bounce**: Bionatus, Laticínios Manchester, The Best Salgados.
- Todos com apresentação enviada no mesmo lote (19 envios no dia — maior volume diário da semana). **10 de 19 são gorduras/laticínios/queijarias** — prospecção segue alinhada ao nicho 1 (vencidos >40%).

### Problemas encontrados e correções
- **Watchdog exit 1 (3 FP2 atrasados)** → resolvido no mesmo tick (backlog zerado). Pendências estruturais mantidas: fila em **0 pendentes** (Prospector repôs 19 hoje — ótima rodada — mas cadência 5-8/dia segue necessária) e Netlify sem créditos (403, GitHub Pages único no ar).
...
### Nota
Tick de correção leve: 3 FP2 atrasados disparados (nicho gorduras/laticínios) e watchdog de volta a exit 0. Caixa limpa (0 respostas pendentes), nenhuma resposta nova de lead, nenhum pedido de relatório ESG (sem geração de PDF). Prospector teve a melhor rodada da semana (+19, sendo 10 laticínios/queijarias). Funil saudável (7,8%, acima da meta de 3%). Ações humanas seguem em aberto: **chamar Rafael (Queijos Itupeva) e toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09)**.
...
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **186** — ativos: **141** (bounces cache: 44). Fila de prospecção: **146 empresas / 135 contatados / 29 bounce / 0 pendentes**.

### Números do estado real (pós-tick 09:02)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 44).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 44 do cache já tentados).
- **Watchdog:** ✅ **exit 0 de primeira** — saudável, sem follow-ups atrasados, bounces sem correção ou leads parados.
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes — 13 FP2 já enviados no tick 08:40).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`) — caixa limpa.
- **Dashboard:** **160 apresentações / 136 entregues / 24 bounces (15%) / 14 respostas / taxa 8,8%** (meta >3%) — funil acima da meta pelo 2º mês.
...
6. **FP1/FP2 de supermercados aguardando resposta (nicho que mais converte):** Dalben, Paulistão, Amigão, Enxuto, Tauste, Chimar (FP1) + Tenda Atacado (FP2 14/09).
7. **Novos no ar (V6 hotéis/resorts, enviados 12/09 14:31):** Fazenda Capoava, Itu Garden, Brisa Itu, Thermas de São Pedro, Dona Carolina — monitorar retorno do nicho hospitalidade.
8. **Andorinha Hiper Center (id 147) — recusa cordial 12/09 12:02:** já tem coletor contratado, guardou o contato para futuro — registrar para recontato no ciclo de renovação do coletor (loja em SALTO).

### Nota
Tick tranquilo (12º consecutivo 100% verde): caixa limpa, zero atrasos, zero respostas pendentes, nenhum bounce novo. Funil segue saudável (8,8% de resposta, acima da meta de 3%). Próximas ações humanas seguem em aberto: toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09) e coleta-teste do Queijos Itupeva.

---

## 2026-09-13 (domingo) — ESTRATEGISTA (planejamento semanal 08:00)

### Diagnóstico da semana (números reais)
- **Funil**: 160 apresentações / 136 entregues / 24 bounces (15%) / **14 respostas reais / taxa 8,8%** (meta >3%) — acima da meta pelo 2º mês ✅
- **Leads**: 186 total / 16 respondidos / 26 sequência / 26 bounce CSV (44 cache, após correção do id 142) / 2 encerrados
- **Fila**: 146 empresas / 135 contatados / 29 bounce / **0 pendentes** (gargalo nº1 mantido)
- **Inbound**: 0 leads novos via site (Netlify sem créditos 403; só GitHub Pages no ar)
- **O VERDADEIRO gargalo**: 14 respostas e ZERO contratos/coletas-teste em 2 meses — **o funil converte, o fechamento não**. Ações humanas vencidas: Savegnago (10/09), Grupo IMC (10/09), Selmi (11/09); Queijos Itupeva aguardando coleta-teste.

### Pesquisa de mercado (13/09, fontes: APROBIO/Exame, Mordor, biodieselbr/Argus, Instagram)
1. **NOVO — Escassez global de UCO**: Brasil passou a exportar óleo usado para os EUA (CSIV aceito por MAPA/MRE). EUA importaram **1,4 mi t em 2023 (~US$ 1,66 bi)**; Europa consome **130 mil barris/dia = 8x o que coleta**; T&E: metas de SAF até 2030 exigirão **o DOBRO do UCO coletável** nos EUA+Europa+China. → "o mundo está disputando óleo usado": argumento de urgência para fechar contrato agora (fonte: https://aprobio.com.br/noticia/oleo-de-cozinha-usado-se-torna-estrategico-e-eua-abrem-mercado-para-o-brasil/)
2. Mordor: UCO global US$ 7,28 bi (2025) → 7,68 bi (2026) — consistente com o US$ 8 bi/Fortune dos templates (mantido).
3. Cotação SP ~R$ 4.825/t cif (Argus) e faixa R$ 1,00–2,50/L ao gerador confirmadas.
4. Ofertas de **R$ 3,50/L** continuam ativas (anúncios Instagram) — confirma alerta do estudo: acima de R$ 2,50 = coletor irregular/leilão; usar o contraste "preço justo + operação regular".

### Melhorias aplicadas
1. **Bug corrigido (sync_formspree.py)**: leads `encerrado`/`respondido` não são mais rebaixados a `bounce` pelo sync (causa-raiz da recidiva do Rede Top id 142 — apontada pelo Analista 12/09). py_compile OK. id 142 corrigido para `encerrado` (bounce CSV 27→26).
2. **persona.md**: novo argumento "ESCASSEZ GLOBAL DE UCO" (exportação EUA/CSIV, 1,4 mi t, T&E dobrar demanda) — Atendente usa em respostas de leads quentes.
3. **FP2 (prospecao_followup.py)**: urgência reforçada com exportação para EUA + Portaria 3/2026. Render OK.

### Metas da semana (14–20/09) — FOCO FECHAMENTO
- **1 coleta-teste CONFIRMADA ou 1 contrato até 20/09** — caminho: Queijos Itupeva (agendar com Rafael, WhatsApp 11 940333759) → Savegnago (toque WhatsApp 11 96785-9631, URGENTE) → Grupo IMC → Selmi → Sumerbol/GoodBom/Rede Boa (toque leve).
- Fila ≥ 10 pendentes (Prospector 5-8/dia, 2 horários; prioridade supermercados/atacarejo + frigoríficos + cozinhas industriais).
- Manter taxa ≥ 8% e monitorar hotéis V6 + FP1 de supermercados.

---

## 2026-09-12 (sábado) — ANALISTA DE QUALIDADE (auditoria fim de dia ~19:35)

### Resumo do dia (fechamento)
- **Pipeline (fechamento 19:35)**: **186 leads** · **115 novos** · 26 sequência · **16 respondidos** · **27 bounce (CSV)** / **44 no cache** · 2 encerrados · **ativos: 157**.
- **Sync do fim de dia** ✅: 44 emails no bounce cache, **0 notificações Formspree** (0 leads novos via site). **Bounces novos hoje: 0** (mesmo conjunto do backup das 10:32 — nada novo entrou no cache).
- **Emails enviados hoje: 23** (confirmado nos timestamps do CSV): **8 FP1** (09:47–09:48, laticínios/queijarias) + **7 FP2** (10:01, refeitórios/rest. industriais) + **8 apresentações** (3 às 09:53 — Pelissari/Quata/Marcy; 5 às 14:32 — V6 hotéis/resorts).
- **Leads novos hoje: 8** (ids 179–186, `prospeccao-lote`).
- **Respostas hoje: 1** — Andorinha Hiper Center (id 147, 12:02, recusa cordial — já atendida pelo Atendente).
- **replies_pending.json = `[]` → 0 respostas pendentes** (caixa limpa; o Atendente não tem nada a responder no próximo tick).

### Auditoria de qualidade (o que foi conferido/corrigido)
1. **bounces.json × leads.csv — CONFIRMADO ✅**: os **27 leads com status `bounce`** têm o email presente no cache; nada a corrigir no match exato. As **17 entradas restantes do cache NÃO correspondem a nenhum email de lead** — são endereços alternativos/originais das mesmas empresas (ex.: `contato@selmi.com.br` bounceou, mas o lead usa `sac@selmi.com.br` e **respondeu**; `sac@redeoba.com.br` bounceou, mas o Oba respondeu por `atendimento@redeoba.com.br`; `3d@roldao.com.br` além do `comercial@` já marcado; variante malformada `ecommerce704@redetop.com.b`). **Nenhuma delas é email atual de lead ativo** → comportamento correto (um match por domínio marcaria Oba/Selmi como bounce por engano).
2. **⚠️ Registro (não corrigido, conforme procedimento)**: **2 leads `bounce` COM `boas_vindas_em` preenchido** (antes era só 1): **id 1** Ana Souza (`ana@alimentossalto.com.br`, bv 13/08) e **id 142** Rede Top (`ecommerce704@redetop.com.br`, bv 07/09 + `follow2_em` 12/09 10:01). **Sem risco de reenvio** — `send_sequence` pula `bounce` (linha 213 do bot). O id 142 é **recidivante** → ver item 3.
3. **🔎 Causa-raiz id 142 (Rede Top) — o "encerrado" não gruda**: o tick das 11:04 registrou a correção (status → `encerrado`), mas o lead aparece **`bounce` de novo**. **Motivo:** `sync_formspree.py` (bloco de correção, ~linha 136) re-marca `bounce` **qualquer** lead cujo email esteja no cache e cujo status ≠ `bounce` — inclusive `encerrado`. Como `ecommerce704@redetop.com.br` continua sendo devolvido pelo mailer-daemon, **todo sync rebaixa o status para `bounce`**. Não é falha do Atendente; é efeito colateral do sync.
4. **Respostas pendentes: 0** (`replies_pending.json` = `[]`, confirmado).
5. **Higiene de dados (re-registrado)**: **14 leads sem `id`** no CSV (frigoríficos Verdi/3R/Frisa/do Sul, Intercity, Arese, Infinity, Veridi, HRT, Larimar, Maila, Farmaviva, Audacci, GDS USA — todos `sequencia`/`bounce`). Não quebra o pipeline, mas atrapalha relatórios e rastreio.
6. **Legado (baixa severidade)**: 7 leads `sequencia` sem `boas_vindas_em` (ids 5,7,8,11,12,13,15 — todos `fonte=prospeccao`, com fluxo outbound próprio) e 1 `respondido` sem `respondido_em` (id 2, lead de teste). **Sem risco de envio duplicado**: `send_sequence` ignora `fonte=prospeccao*`.

### Sugestões para o Estrategista
1. **Corrigir o sync para respeitar `encerrado`** (causa-raiz do item 3): em `sync_formspree.py`, ao marcar bounces, **pular leads com status `encerrado`** (ou introduzir um status terminal tipo `invalidado` que o sync nunca sobrescreve). Hoje o Rede Top (id 142) volta a `bounce` a cada tick e infla o número de "bounces CSV".
2. **Trocar os endereços que deram bounce por padrão alternativo validado** onde há indício de contato vivo: **Selmi** (`contato@` bounce → usar `compras@`/`sac@`, já respondidos); **Oba** (`sac@` bounce → `atendimento@` funcionou); **Cowpig** (`contato@` bounce → lead ativo usa `atendimento@`); **Penina** (`contato@` bounce → lead usa `info@`). Revalidar MX antes de reenviar.
3. **Arquivar/rotacionar `equipe/insights.md`**: já em **~500 KB / 4.628 linhas** e crescendo ~1 entrada por tick — a leitura por ferramentas já falha (detecção "binária"). Sugiro mover o histórico anterior a 09/2026 para `equipe/insights-arquivo.md` e **consolidar** com `bot/equipe/insights.md` (hoje há duas árvores `equipe/` e `bot/equipe/` divergindo).
4. **Backfill de IDs**: atribuir IDs únicos aos 14 leads sem `id` (rodada rápida de higiene).

### Nota
Dia estável na rotina (0 respostas pendentes, 0 bounces novos, caixa limpa, 23 envios). A única pendência com efeito real é a **recidiva do Rede Top (id 142)** causada pelo sync, mais a **higiene de dados** (IDs vazios) — ambas de baixa urgência, porém recorrentes. Ações humanas seguem em aberto: toques de WhatsApp vencidos (Savegnago, Grupo IMC, Selmi) e coleta-teste do Queijos Itupeva.

---

## 2026-09-12 (sábado) — ATENDENTE (tick 18:02)

### ✅ Tick limpo — pipeline 100% verde (watchdog exit 0 de primeira, 11º tick consecutivo)
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **186** — ativos: **141** (bounces cache: 44). Fila de prospecção: **146 empresas / 135 contatados / 29 bounce / 0 pendentes**.

### Números do estado real (pós-tick 18:02)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 44).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 44 do cache já tentados).
- **Watchdog:** ✅ **exit 0 de primeira** — saudável, sem follow-ups atrasados, bounces sem correção ou leads parados.
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
...
### 🆕 Novidades na caixa
- **Nenhuma resposta nova** desde a Andorinha (12:02, recusa cordial — já atendida). Caixa limpa, zero respostas órfãs, zero pendências. **Sem pedidos de relatório ESG/certificado** — sem geração de PDF neste tick.
...
4. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **PRAZO 11/09 VENCIDO → toque leve se silêncio** (canal comercial aberto desde 08/09).
5. **Confiança Supermercados — STAND BY (não insistir):** recontatar em ~2–3 meses citando protocolo 236485.
6. **FP1 de supermercados aguardando resposta (nicho que mais converte):** Dalben, Paulistão, Amigão, Enxuto, Tauste, Chimar.
7. **Novos no ar (V6 hotéis/resorts, enviados 14:31):** Fazenda Capoava, Itu Garden, Brisa Itu, Thermas de São Pedro, Dona Carolina — monitorar retorno do nicho hospitalidade.
8. **Andorinha Hiper Center (id 147) — recusa cordial 12:02:** já tem coletor contratado, guardou o contato para futuro — registrar para recontato no ciclo de renovação do coletor (loja em SALTO).

### Nota
Tick tranquilo (11º consecutivo 100% verde): caixa limpa, zero atrasos, zero respostas pendentes, nenhum bounce novo. Funil segue saudável (8,8% de resposta, acima da meta de 3%). Próximas ações humanas seguem em aberto: toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09) e coleta-teste do Queijos Itupeva.

## 2026-09-12 (sábado) — ATENDENTE (tick 16:31)

### ✅ Tick limpo — pipeline 100% verde (watchdog exit 0 de primeira, 10º tick consecutivo)
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **186** — ativos: **141** (bounces cache: 44). Fila de prospecção: **146 empresas / 135 contatados / 29 bounce / 0 pendentes**.
...
### Nota
Tick tranquilo (10º consecutivo 100% verde): caixa limpa, zero atrasos, zero respostas pendentes, nenhum bounce novo. Funil segue saudável (8,8% de resposta, acima da meta de 3%). Próximas ações humanas seguem em aberto: toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09) e coleta-teste do Queijos Itupeva.

## 2026-09-12 (sábado) — ATENDENTE (tick 16:01)

### ✅ Tick limpo — pipeline 100% verde (watchdog exit 0 de primeira, 9º tick consecutivo)
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **186** — ativos: **141** (bounces cache: 44). Fila de prospecção: **146 empresas / 135 contatados / 29 bounce / 0 pendentes**.

### Números do estado real (pós-tick 16:01)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 44).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 44 do cache já tentados).
- **Watchdog:** ✅ **exit 0 de primeira** — saudável, sem follow-ups atrasados, bounces sem correção ou leads parados.
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
...
### Nota
Tick tranquilo (9º consecutivo 100% verde): caixa limpa, zero atrasos, zero respostas pendentes, nenhum bounce novo. Funil segue saudável (8,8% de resposta, acima da meta de 3%). Próximas ações humanas seguem em aberto: toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09) e coleta-teste com Queijos Itupeva.

---

## 2026-09-12 (sábado) — ATENDENTE (tick 15:32)

### ✅ Tick limpo — pipeline 100% verde (watchdog exit 0 de primeira, 8º tick consecutivo)
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **186** — ativos: **141** (bounces cache: 44). Fila de prospecção: **146 empresas / 135 contatados / 29 bounce / 0 pendentes**.

### Números do estado real (pós-tick 15:32)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 44).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 44 do cache já tentados).
- **Watchdog:** ✅ **exit 0 de primeira** — saudável, sem follow-ups atrasados, bounces sem correção ou leads parados.
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`) — caixa limpa.
- **enviar_lote --status:** ✅ fila confirmada **146/135/29/0** — sem pendentes (8 envios no dia: Pelissari/Quata/Marcy 09:53 + 5 hotéis/resorts V6 14:31).
- **Dashboard:** **160 apresentações / 136 entregues / 24 bounces (15%) / 14 respostas / taxa 8,8%** (meta >3%) — funil acima da meta pelo 2º mês.
...
### Nota
Tick tranquilo (8º consecutivo 100% verde): caixa limpa, zero atrasos, zero respostas pendentes, nenhum bounce novo. Funil segue saudável (8,8% de resposta, acima da meta de 3%). Próximas ações humanas seguem em aberto: toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09) e coleta-teste com Queijos Itupeva.

---

## 2026-09-12 (sábado) — ATENDENTE (tick 14:31)

### ✅ Tick limpo — pipeline 100% verde (watchdog exit 0 de primeira, 7º tick consecutivo)
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **186** — ativos: **136** (bounces cache: 44). Fila de prospecção consumida: **5 envios V6 de hotéis/resorts**.

### Números do estado real (pós-tick 14:31)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 44).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos os 44 do cache já tentados).
- **Watchdog:** ✅ **exit 0 de primeira** — saudável, sem follow-ups atrasados, bounces sem correção ou leads parados.
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`) — caixa limpa.
- **enviar_lote:** ✅ **5/5 enviados** (pendentes do Melhorador 14:10): Fazenda Capoava (Itu), Itu Garden SPA Resort (Itu), Hotel Fazenda Brisa Itu (Itu), Thermas de São Pedro (São Pedro), Hotel Fazenda Dona Carolina (Itatiba) — todos template **V6 hotéis/resorts**, MX validado, sem bounce imediato. Fila agora **146 empresas / 135 contatados / 29 bounce / 0 pendentes**.
- **Dashboard:** **160 apresentações / 136 entregues / 24 bounces (15%) / 14 respostas / taxa 8,8%** (meta >3%) — funil acima da meta pelo 2º mês; 8 envios no dia (12/09): Pelissari/Quata/Marcy 09:53 + 5 hotéis 14:31.

### 🆕 Novidades na caixa
- **Nenhuma resposta nova** desde a Andorinha (12:02, recusa cordial — já atendida). Caixa limpa, zero respostas órfãs, zero pendências. **Sem pedidos de relatório ESG/certificado** — sem geração de PDF neste tick.

### Problemas encontrados e correções
- **Nenhum problema neste tick.** Pendências estruturais mantidas: fila de prospecção em **0 pendentes** (gargalo do Prospector — precisa repor 5-8/dia com MX validado, mesmo após os +5 de hoje) e Netlify sem créditos (403, GitHub Pages segue como único no ar).

### Leads quentes (destaque — ação humana)
1. **🔥 QUEIJOS ITUPEVA (NEGOCIAÇÃO ATIVA desde 16:30 11/09):** Rafael Galvão pediu negociação no WhatsApp (11 940333759) → **AÇÃO HUMANA: chamar** (nicho vencidos >40% gordura; Frescal 200 kg/mês + manteiga/requeijão/doce 50–100 kg/mês; referência R$0,50–1,50/kg conforme dificuldade de separação — queijos = separação difícil, tender ao piso; **descaracterização é cortesia, não cobrada**).
2. **Savegnago Supermercados (~100 lojas) — #1:** **PRAZO 10/09 VENCIDO → toque no WhatsApp (11) 96785-9631 (ação humana URGENTE, independente do bot).**
3. **Frango Assado / Grupo IMC — #2:** **PRAZO 10/09 VENCIDO → toque no WhatsApp (11) 96785-9631.**
4. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **PRAZO 11/09 VENCIDO → toque leve se silêncio** (canal comercial aberto desde 08/09).
5. **Confiança Supermercados — STAND BY (não insistir):** recontatar em ~2–3 meses citando protocolo 236485.
...
### Nota
Tick tranquilo (7º consecutivo 100% verde): caixa limpa, fila consumida (5 envios V6 de hotéis/resorts), zero atrasos, zero respostas pendentes, nenhum bounce novo. Funil segue saudável (8,8% de resposta, acima da meta de 3%). Próximas ações humanas seguem em aberto: toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09) e coleta-teste com Queijos Itupeva.

---

## 2026-09-12 (sábado) — MELHORADOR CONTÍNUO (tick 14:xx)

### 🎯 Diagnóstico do dia
- **Watchdog**: exit 0 — saudável (SMTP OK · IMAP OK), 181 leads / 136 ativos / 44 bounces cache, 0 atrasos.
- **Dashboard**: 155 apresentações / 131 entregues / 24 bounces (15%) / **14 respostas / taxa 9,0%** (acima da meta >3%).
- **PONTO MAIS FRACO: FILA ZERADA (0 pendentes)** — causa sistêmica conhecida: Atendente consome ~3/dia, Prospector repõe 2-4/dia → fila vive no zero.

### ✅ Melhorias implementadas (12/09)
1. **Fila reposta +5 hotéis/resorts com MX validado** (`bot/fila_prospeccao_extra.json`, 110 → 115):
   - Fazenda Capoava (`reservas@fazendacapoava.com.br`, KingHost MX) — Itu/SP, hotel-fazenda pensão completa.
   - Itu Garden SPA Resort (`reservas@itugardenspa.com.br`, Locaweb MX) — Itu/SP.
   - Hotel Fazenda Brisa Itu (`reservas@hotelfazendabrisaitu.com.br`, Outlook MX) — Itu/SP.
   - Thermas de São Pedro (`reservas@saopedrothermasresort.com.br`, Outlook MX) — São Pedro/SP.
   - Hotel Fazenda Dona Carolina (`reservas@donacarolina.com.br`, Locaweb MX) — Itatiba/SP.
   - Dedup contra 4 fontes (fila + leads.csv + bounces + EMPRESAS = 198 emails únicos) → todos OK. Verificação: `enviar_lote.py --status` → **pendentes: 5** (exatamente os novos) e `--dry-run` confirma os 5 passando pelo gate MX do script.
   - **Descartados com critério**: San Raphael Country e Limoeiro da Concórdia (MX self-hosted, porta 25 timeout → bounce quase certo), União Supermercados uniaosm.com.br (SEM MX).
   - **Nicho novo coberto: hotéis-fazenda de Itu** — cozinha industrial de pensão completa (café/almoço/jantar todos os dias = fritura constante o ano todo), contrato único de coleta, sensibilidade ESG do setor. Mesma lógica dos hotéis urbanos (Royal Palm etc.), agora no polo turístico de Itu.
2. **Branch V6 de template para hotéis/resorts** (`bot/prospecao.py`): palavras-chave `hotel/resort/pousada/pensao/hospedagem/spa` disparam ângulo próprio — fritura diária da pensão completa + US$ 8,6 bi + PNRS + relatório ESG (argumento forte p/ hospitalidade) + urgência Portaria MME/MMA 3/2026 + CTA litros. `enviar_lote.py` delega ao prospecao (fonte única confirmada) → o próximo lote já usa o V6.
3. **Testes**: `py_compile` OK + script de verificação cobre V6 hotel/resort + regressão V4 supermercados / V3 vencidos / V5 máquinas / V2 genérico — **todos passaram**.

### 📌 Para o Estrategista dominical
- Ritmo do Prospector segue insuficiente: repor 5-8/dia (ou 2 horários) em vez de 2-4 — fila vive zerada mesmo com os +5 de hoje.
- Toques WhatsApp vencidos: Savegnago (10/09), Grupo IMC (10/09), Selmi (11/09) — ação humana.
- Queijos Itupeva segue #1 (negociação ativa, aguardando relação + coleta-teste).

---
## 2026-09-12 (sábado) — ATENDENTE (tick 13:31)

### ✅ Tick limpo — pipeline 100% verde (watchdog exit 0 de primeira)
- **Estado**: watchdog **exit 0 direto** (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **181** — ativos: **136** (bounces cache: 44). 6º tick consecutivo saudável.
...
- **enviar_lote --status:** 141 empresas / 130 contatados / 29 bounce / **0 pendentes** (fila zerada).
- **Dashboard:** **155 apresentações / 131 entregues / 24 bounces (15%) / 14 respostas / taxa 9,0%** (meta >3%) — funil acima da meta pelo 2º mês; 3 apresentações no dia (12/09, 09:53: Pelissari, Quata, Marcy).

### 🆕 Novidades na caixa
- **Nenhuma resposta nova** desde a Andorinha (12:02, recusa cordial — já atendida no tick 12:05). Caixa limpa, zero respostas órfãs, zero pendências.

### Ações do tick
- Rotina completa executada: sync → corrigir → watchdog → followup → sequência → check-replies. Tudo verde.
- Nenhum pedido de relatório ESG/certificado de impacto — **sem geração de PDF** neste tick.

### Problemas encontrados e correções
- **Nenhum problema neste tick.** Pendências estruturais mantidas: fila de prospecção em **0 pendentes** (gargalo do Prospector — precisa repor 5-8/dia com MX validado) e Netlify sem créditos (403, GitHub Pages segue como único no ar).

### Leads quentes (destaque — ação humana)
1. **🔥 QUEIJOS ITUPEVA (NEGOCIAÇÃO ATIVA desde 16:30 11/09):** Rafael Galvão pediu negociação no WhatsApp (11 940333759) → **AÇÃO HUMANA: chamar HOJE** (nicho vencidos >40% gordura; Frescal 200 kg/mês + manteiga/requeijão/doce 50–100 kg/mês; referência R$0,50–1,50/kg conforme dificuldade de separação — queijos = separação difícil, tender ao piso; **descaracterização é cortesia, não cobrada**).
2. **Savegnago Supermercados (~100 lojas) — #1:** **PRAZO 10/09 VENCIDO → toque no WhatsApp (11) 96785-9631 (ação humana URGENTE, independente do bot).**
3. **Frango Assado / Grupo IMC — #2:** **PRAZO 10/09 VENCIDO → toque no WhatsApp (11) 96785-9631.**
4. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **PRAZO 11/09 VENCIDO → toque leve se silêncio** (canal comercial aberto desde 08/09).
5. **Confiança Supermercados — STAND BY (não insistir):** recontatar em ~2–3 meses citando protocolo 236485.
6. **FP1 de supermercados aguardando resposta (nicho que mais converte):** Dalben, Paulistão, Amigão, Enxuto, Tauste, Chimar.
7. **Andorinha Hiper Center (id 147) — recusa cordial 12/02:** já tem coletor contratado, guardou o contato para futuro — registrar para recontato no ciclo de renovação do coletor (loja em SALTO).

### Nota
Tick tranquilo (6º consecutivo 100% verde): caixa limpa, fila zerada, zero atrasos, zero respostas pendentes. Sem novidades desde a recusa cordial da Andorinha. Próximas ações humanas seguem em aberto: toques de WhatsApp vencidos (Savegnago 10/09, Grupo IMC 10/09, Selmi 11/09) e coleta-teste com Queijos Itupeva.

---

## 2026-09-12 (sábado) — ATENDENTE (tick 12:31)

### ✅ Tick limpo — pipeline 100% verde (watchdog exit 0)
- **Estado**: watchdog **exit 0** direto (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **181** — ativos: **136** (bounces cache: 44).

### Números do estado real (pós-tick 12:31)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 44).
...
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`).
- **enviar_lote --status:** 141 empresas / 130 contatados / 29 bounce / **0 pendentes**.
- **Dashboard:** **155 apresentações / 131 entregues / 24 bounces (15%) / 14 respostas / taxa 9,0%** (meta >3%) — funil acima da meta pelo 2º mês; 3 apresentações no dia (12/09, 09:53: Pelissari, Quata, Marcy).

### 🆕 Resposta nova no funil (verificada via IMAP)
- **Andorinha Hiper Center Supermercados** (sugestoes@andorinhahiper.com.br, id 147) respondeu às **11:46**: *"já temos uma empresa que recolhe o óleo e no momento não temos intenção de trocar... ficaremos com seu contato caso haja interesse futuro"* — resposta **negativa cordial** (não é lead quente). O bot já respondeu às **12:08** com encerramento elegante (agradeceu a transparência, reforçou coleta programada + certificado PNRS + pagamento PIX, deixou WhatsApp (11) 96785-9631 aberto para futuro). Lead marcado como `respondido` — consistente, sem resposta órfã.

### Ações do tick
- Verificação extra de integridade (IMAP) da resposta Andorinha: enviada às 12:08 na pasta Enviados, conteúdo alinhado à persona (regra 5: sem crítica a concorrente, porta aberta, CTA claro). Nenhuma ação corretiva necessária.
- Nenhum pedido de relatório ESG/certificado de impacto — **sem geração de PDF** neste tick.
...
3. **Frango Assado / Grupo IMC — #2:** **PRAZO 10/09 VENCIDO → toque no WhatsApp (11) 96785-9631.**
4. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **PRAZO 11/09 VENCIDO → toque leve se silêncio** (canal comercial aberto desde 08/09).
5. **Confiança Supermercados (id 171):** proposta enviada a **social@confianca.com.br** (SAC protocolo 236485); rede = nicho que mais converte. Acompanhar retorno; toque WhatsApp se silêncio em 3 dias.
6. **Lopes Supermercados:** canal comercial explícito (FAQ oficial pede apresentação de fornecedores) — potencial de resposta alto.
7. **Rede Bom Lugar, Tenda Atacado, Roldão** — aguardando; **Sumerbol (Ivone), GoodBom (Laura), Rede Boa (#23915), Arcor/Bagley, Sanofi (02995121)** — em aberto. **Andorinha:** banco para reativação futura (não é lead quente).

### Nota
Tick 100% operacional: watchdog verde, caixa limpa confirmada (check-replies: 0 pendentes + verificação IMAP da resposta Andorinha), fila com 0 pendentes, nenhum follow-up atrasado, nenhum bounce novo. Funil segue saudável e **subiu para 9,0% de resposta** (14 respostas, +1 no dia: Andorinha — negativa cordial). Ação humana pendente: **chamar Rafael (Queijos Itupeva) hoje, toque no WhatsApp para Savegnago e Grupo IMC (prazo 10/09 vencido) e Selmi (prazo 11/09 vencido)**.

---

## 2026-09-12 (sábado) — ATENDENTE (tick 11:31)

### ✅ Tick limpo — pipeline 100% verde (watchdog exit 0)
- **Estado**: watchdog **exit 0** direto (`SMTP OK · IMAP OK`). **Nada a corrigir neste tick.** Leads totais: **181** — ativos: **137** (bounces cache: 44).

### Números do estado real (pós-tick 11:31)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site; cache 44).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos já tentados).
- **Watchdog:** ✅ **exit 0** — saudável, sem follow-ups atrasados, bounces sem correção ou leads parados.
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`) — caixa limpa confirmada.
- **enviar_lote --status:** 141 empresas / 130 contatados / 29 bounce / **0 pendentes**.
- **Dashboard:** **155 apresentações / 131 entregues / 24 bounces (15%) / 13 respostas / taxa 8,4%** (meta >3%) — funil acima da meta pelo 2º mês; 3 apresentações no dia (12/09, 09:53: Pelissari, Quata, Marcy).

### Ações do tick
- Nenhum follow-up atrasado, nenhuma resposta pendente, nenhum pedido de relatório ESG/certificado de impacto — **nenhuma ação corretiva necessária** (tick 100% operacional, sem geração de PDF).
...
### Nota
Tick 100% operacional: watchdog verde, caixa limpa confirmada (check-replies: 0 pendentes), fila com 0 pendentes, nenhum follow-up atrasado, nenhum bounce novo, nenhuma resposta órfã. Funil segue saudável (8,4% de resposta, acima da meta de 3%). Ação humana pendente: **chamar Rafael (Queijos Itupeva) hoje, toque no WhatsApp para Savegnago e Grupo IMC (prazo de 10/09 vencido) e Selmi (prazo 11/09 vencido)**.
...
- **Estado**: watchdog **exit 0** direto (`SMTP OK · IMAP OK` — 10º tick verde de credencial após restauração de 10/09 16:08). **Nada a corrigir neste tick.**
- **Leads totais:** **176** — ativos: **133** (bounces cache: 42).
...
### Nota
Tick operacional sem intercorrências: watchdog verde (10º seguido), caixa limpa (0 pendentes), nenhum bounce novo, nenhuma resposta órfã. Funil segue saudável (8,7% de resposta). Ação humana pendente: **toque no WhatsApp para Savegnago e Grupo IMC (prazo de 10/09 vencido)** e **Selmi hoje (11/09)**.

---

## 2026-09-11 (sexta) — ATENDENTE (tick 11:31)

### ✅ Tick limpo — pipeline 100% verde (9º tick seguido de credencial OK)
- **Estado**: watchdog **exit 0** direto (`SMTP OK · IMAP OK` — 9º tick verde de credencial após restauração de 10/09 16:08). **Nada a corrigir neste tick.**
- **Leads totais:** **176** — ativos: **133** (bounces cache: 42).
...
### Nota
Tick operacional sem intercorrências: watchdog verde (9º seguido), caixa limpa (0 pendentes), nenhum bounce novo, nenhuma resposta órfã. Funil segue saudável (8,7% de resposta). Ação humana pendente: **toque no WhatsApp para Savegnago e Grupo IMC (prazo de 10/09 vencido)** e **Selmi hoje (11/09)**.

---

## 2026-09-11 (sexta) — ATENDENTE (tick 11:02)

### ✅ Tick limpo — pipeline 100% verde (8º tick seguido de credencial OK)
- **Estado**: watchdog **exit 0** direto (`SMTP OK · IMAP OK` — 8º tick verde de credencial após restauração de 10/09 16:08). **Nada a corrigir neste tick.**
- **Leads totais:** **176** — ativos: **133** (bounces cache: 42).

### Números do estado real (pós-tick 11:02)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos já tentados; cache 42).
- **Watchdog:** ✅ **exit 0** — saudável, sem follow-ups atrasados (backlog de 10:32 já estava zerado).
- **prospecao_followup:** ✅ 0 follow-ups atrasados — nenhum envio necessário.
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`).
- **enviar_lote --status:** 136 empresas / 125 contatados / 29 bounce / **0 pendentes**.
- **Dashboard:** **150 apresentações / 126 entregues / 24 bounces (16%) / 13 respostas / taxa 8,7%** (meta >3%) — avanço de +2 apresentações e +1 resposta desde o tick 10:32; funil acima da meta pelo 2º mês.
...
### Nota
Tick operacional sem intercorrências: watchdog verde, caixa limpa (0 pendentes), nenhum bounce novo, nenhuma resposta órfã. Funil segue saudável (8,7% de resposta). Ação humana pendente: **toque no WhatsApp para Savegnago e Grupo IMC (prazo de 10/09 vencido)** e **Selmi hoje (11/09)**.
...
## 2026-09-11 (sexta) — ATENDENTE (tick 10:32)

### ✅ Watchdog com problema resolvido — pipeline verde de novo
- **Estado**: watchdog acusou **3 follow-ups FP1 ATRASADOS** (exit 1) — corrigido na hora rodando `prospecao_followup.py`: **3/3 FP1 enviados** (Sorocaba Frios, Mega Ingredientes, Grupo Lanchero). Email 100% OK (SMTP OK · IMAP OK — 7º tick verde de credencial após a restauração de 10/09 16:08).
- **Leads totais:** **176** — ativos: **133** (bounces cache: 42).

### Números do estado real (pós-tick 10:32)
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos já tentados; cache 42).
- **Watchdog:** ✅ **resolvido** — exit 1 no início (3 FP1 atrasados) → exit 0 após o follow-up (backlog zerado).
- **prospecao_followup:** ✅ **3 FP1 enviados** — Sorocaba Frios (`contato@sorocabafrios.com.br`), Mega Ingredientes (`contato@megasabor.com.br`), Grupo Lanchero (`sac@lanchero.com.br`, indústria de maioneses = nicho vencidos >40% gordura).
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`).
- **enviar_lote --status:** 136 empresas / 125 contatados / 29 bounce / **0 pendentes**.
- **Dashboard:** **148 apresentações / 124 entregues / 24 bounces (16%) / 12 respostas / taxa 8,1%** (meta >3%) — funil segue acima da meta, sem apresentações novas neste tick (só follow-ups).

### Ações do tick
- **3 follow-ups FP1 enviados** (backlog do watchdog resolvido). Nenhum outro envio pendente.
- Nenhum pedido de relatório ESG/certificado de impacto — sem geração de PDF.

### Problemas encontrados e correções
- **Único problema do tick (resolvido):** 3 follow-ups FP1 atrasados — disparados automaticamente por `prospecao_followup.py` com sucesso (3/3). Nenhum bounce novo, nenhuma resposta órfã.
- Pendências estruturais mantidas: fila de prospecção em **0 pendentes** (gargalo do Prospector — precisa repor 5-8/dia com MX validado; hoje ele já enviou Fugini Alimentos e Grow Química) e Netlify sem créditos (403, GitHub Pages segue como único no ar).

### Leads quentes (destaque — ação humana)
1. **Savegnago Supermercados (~100 lojas) — #1:** **PRAZO 10/09 VENCEU → toque no WhatsApp (11) 96785-9631 (ação humana URGENTE, independente do bot).**
2. **Frango Assado / Grupo IMC — #2:** **PRAZO 10/09 VENCEU → toque no WhatsApp (11) 96785-9631.**
3. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **PRAZO 11/09 (HOJE) → toque leve se silêncio** (canal comercial aberto desde 08/09).
4. **Confiança Supermercados (id 171) — mais quente do dia:** proposta enviada a **social@confianca.com.br** (SAC protocolo 234776, Larissa Arruda); rede = nicho que mais converte. Acompanhar retorno; toque WhatsApp se silêncio em 3 dias.
5. **Queijos Itupeva** — respondeu 10/09 16:13 (aguardando volume mensal + tipo), nicho vencidos >40% gordura.
6. **Rede Bom Lugar, Tenda Atacado, Roldão** — aguardando; **Sumerbol (Ivone), GoodBom (Laura), Rede Boa (#23915), Arcor/Bagley, Sanofi (02995121)** — em aberto.

### Nota
Tick com 1 problema detectado pelo watchdog (3 FP1 atrasados) e **corrigido na hora**: follow-ups enviados, caixa limpa (0 pendentes), fila com 0 pendentes, nenhum bounce novo. Dia segue aguardando retornos dos canais comerciais abertos. Ação humana pendente: **toque no WhatsApp para Savegnago e Grupo IMC (prazo de 10/09 vencido)** e **Selmi hoje (11/09)**.

---

### ✅ Pipeline 100% verde (6º tick seguido)
- **Estado**: credencial restaurada segue estável — watchdog **exit 0** com `SMTP OK · IMAP OK`. Caixa verificada: **0 respostas pendentes** (`replies_pending.json` = `[]`).

### Números do estado real (pós-tick 08:49)
- **Leads totais:** **174** — ativos: **132** (bounces cache: 42; CSV: 105 novo · 27 sequencia · 26 bounce · 14 respondido · 2 encerrado).
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos já tentados; cache 42).
- **Watchdog:** ✅ **exit 0** — SMTP OK · IMAP OK; nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup:** ✅ 0 follow-ups processados (nada atrasado — backlog zerado desde a restauração).
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`).
- **enviar_lote --status:** 134 empresas / 123 contatados / 29 bounce / **0 pendentes**.
- **Dashboard:** **148 apresentações / 124 entregues / 24 bounces (16%) / 12 respostas / taxa 8,1%** (meta >3%) — funil segue acima da meta, sem envios novos neste tick.

### Ações do tick
- Nenhum email enviado (nada pendente — fila zerada, follow-ups em dia, caixa limpa).
- Nenhum pedido de relatório ESG/certificado de impacto — sem geração de PDF.

### Problemas encontrados e correções
- **Nenhum problema novo.** Sistema 100% operacional pelo 6º tick após o fim do bloqueio de 26 ticks. Pendências estruturais mantidas: fila de prospecção em 0 pendentes (gargalo do Prospector — precisa repor 5-8/dia com MX validado) e Netlify sem créditos (403, GitHub Pages segue como único no ar).
...
### Nota
Tick 100% verde: watchdog exit 0, caixa limpa (0 pendentes), fila com 0 pendentes, nenhum follow-up atrasado, nenhum bounce novo. Nada a responder — dia segue aguardando retornos dos canais comerciais abertos. Ação humana pendente: **toque no WhatsApp para Savegnago e Grupo IMC (prazo de 10/09 vencido)** e **Selmi hoje (11/09)**.

---

## 2026-09-10 (quinta) — ANALISTA DE QUALIDADE (revisão do dia)

### Números do dia (estado real verificado)
- **Sync Formspree:** ✅ executado — **42 emails no bounce cache** (todos correspondentes a leads marcados como bounce) e **0 notificações novas** do Formspree (nenhum lead novo via site hoje).
- **Leads totais:** **174** — **novo: 105 · sequencia: 27 · bounce: 26 · respondido: 14 · encerrado: 2**.
- **Respostas pendentes:** **0** (`replies_pending.json` = `[]`) — Atendente sem fila de resposta no próximo tick.
- **Emails enviados hoje:** 0 (fila zerada, follow-ups em dia, caixa limpa — confirmado nos ticks do Atendente).
- **Dashboard:** 148 apresentações / 124 entregues / 24 bounces (16%) / 12 respostas / taxa 8,1% (meta >3%).

### Auditoria de qualidade (o que foi conferido)
- **a) bounces.json × leads.csv:** ✅ consistente. Os 42 emails do cache dividem-se em: **26** presentes no CSV com status `bounce` (bate exatamente com os 26 leads bounce) e **16** de endereços antigos já corrigidos/substituídos no CSV (históricos — o cache só impede reenvio; o email atual do lead é outro). Nenhum email do cache corresponde a lead com status ≠ bounce → o sync já faz a correção automaticamente.
- **b) Bounce com boas_vindas_em preenchido:** ⚠️ **1 caso registrado** — lead id 1 (ana@alimentossalto.com.br, lead de teste fictício) tem `boas_vindas_em = 2026-08-13T11:31:14-03:00` com status bounce. Apenas registro (o send_sequence pula bounces corretamente; caso é de lead de teste, sem ação).
- **c) Pendências estruturais conhecidas:** 14 linhas sem id no CSV (leads de sequência do lote mais recente) e 1 respondido sem `respondido_em` (id 2, lead de teste Julio). Sem impacto operacional.

### Problemas encontrados e correções
1. **Nenhum problema novo detectado.** Sync OK, bounces consistentes, caixa limpa, fila zerada.
2. **Ocorrência registrada (sem correção):** lead de teste id 1 (ana@alimentossalto.com.br) ficou com status bounce + boas_vindas_em preenchido — não afeta envios reais, mas serve de alerta: leads de teste devem ser excluídos da base ou marcados `encerrado`.
3. Pendência estrutural mantida (já conhecida): **14 linhas sem id** no leads.csv — recomenda-se backfill de ids para rastreabilidade (o watchdog não alarma, mas dificulta correlação com fila/correções).

### Sugestões de melhoria (para o Estrategista)
1. **Backfill de ids vazios no leads.csv** (14 linhas) — fechar lacuna de rastreabilidade; usar os ids 175+ para não colidir com os existentes.
2. **Limpar/arquivar leads de teste** (ids 1, 2, 3) — evitam ruído em auditorias futuras (bounce com boas_vindas_em, respondido sem data).
3. **Canal de prospecção segue sendo o gargalo**: fila com 0 pendentes e 105 leads ainda em status `novo` — o Prospector precisa repor 5-8 empresas/dia com MX validado para o funil não estagnar.

### Nota
Revisão do dia 100% verde: sync executado sem bounces novos, 42 emails no cache 100% consistentes com o CSV, 0 respostas pendentes, nenhum lead com status incorreto além do lead de teste já conhecido. Pipeline estável pelo 5º tick após o fim do bloqueio de credencial. Ações humanas pendentes continuam as mesmas: **toque no WhatsApp de Savegnago e Grupo IMC (prazo 10/09 vencido)** e **Selmi em 11/09**.

---

## 2026-09-10 (quinta) — ATENDENTE (tick 19:31)

### ✅ Pipeline 100% verde (5º tick seguido)
- **Estado**: credencial restaurada segue estável — watchdog **exit 0** com `SMTP OK · IMAP OK`. Caixa verificada: **0 respostas pendentes** (`replies_pending.json` = `[]`).

### Números do estado real (pós-tick 19:31)
- **Leads totais:** **174** — ativos: **132** (bounces cache: 42).
- **Sync Formspree:** ✅ OK — 0 notificações novas (0 leads novos via site).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos já tentados; cache 42).
- **Watchdog:** ✅ **exit 0** — SMTP OK · IMAP OK; nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup:** ✅ 0 follow-ups processados (nada atrasado).
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`).
- **enviar_lote --status:** 134 empresas / 123 contatados / 29 bounce / **0 pendentes**.
- **Dashboard:** **148 apresentações / 124 entregues / 24 bounces (16%) / 12 respostas / taxa 8,1%** (meta >3%) — funil segue acima da meta, sem envios novos neste tick.
...
### Problemas encontrados e correções
- **Nenhum problema novo.** Sistema 100% operacional pelo 5º tick após o fim do bloqueio de 26 ticks. Pendências estruturais mantidas: fila de prospecção em 0 pendentes (gargalo do Prospector — precisa repor 5-8/dia com MX validado) e Netlify sem créditos (403, GitHub Pages segue como único no ar).

### Leads quentes (destaque — ação humana)
1. **Savegnago Supermercados (~100 lojas) — #1:** **PRAZO 10/09 VENCEU HOJE → toque no WhatsApp (11) 96785-9631 (ação humana URGENTE, independente do bot).**
2. **Frango Assado / Grupo IMC — #2:** **PRAZO 10/09 VENCEU HOJE → toque no WhatsApp (11) 96785-9631.**
3. **Confiança Supermercados (id 171) — mais quente do dia:** proposta enviada a **social@confianca.com.br** (SAC protocolo 234776, Larissa Arruda); rede = nicho que mais converte. Acompanhar retorno; toque WhatsApp se silêncio em 3 dias.
4. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **Prazo: 11/09 (amanhã) → toque leve se silêncio.**
5. **Queijos Itupeva** — respondeu 16:13 (aguardando volume mensal + tipo), nicho vencidos >40% gordura.
6. **Rede Bom Lugar, Tenda Atacado, Roldão** — aguardando; **Sumerbol (Ivone), GoodBom (Laura), Rede Boa (#23915), Arcor/Bagley, Sanofi (02995121)** — em aberto.
...
## 2026-09-10 (quinta) — ATENDENTE (tick 19:01)

### ✅ Pipeline 100% verde (4º tick seguido)
- **Estado**: credencial restaurada segue estável — watchdog **exit 0** com `SMTP OK · IMAP OK`. Caixa verificada: **0 respostas pendentes** (`replies_pending.json` = `[]`).
...
- **check-replies:** ✅ 0 respostas aguardando (`replies_pending.json` = `[]`).
- **enviar_lote --status:** 134 empresas / 123 contatados / 29 bounce / **0 pendentes**.
- **Dashboard:** **148 apresentações / 124 entregues / 24 bounces (16%) / 12 respostas / taxa 8,1%** (meta >3%) — funil convertendo acima da meta.
...
### Problemas encontrados e correções
- **Nenhum problema novo.** Sistema 100% operacional pelo 4º tick após o fim do bloqueio de 26 ticks. Pendências estruturais mantidas: fila de prospecção em 0 pendentes (gargalo do Prospector — precisa repor 5-8/dia com MX validado) e Netlify sem créditos (403, GitHub Pages segue como único no ar).
...
### Nota
Tick tranquilo e estável (4º verde seguido): nada pendente, nada atrasado, caixa limpa, sem respostas novas para atender. O foco continua no **fechamento humano** — prazos de WhatsApp de Savegnago e Grupo IMC venceram hoje (10/09), Selmi vence amanhã (11/09). Fila de prospecção segue em 0 pendentes — reposição do Prospector é o único gargalo estrutural restante.

---

## 2026-09-10 (quinta) — ATENDENTE (tick 18:31)

### ✅ Pipeline 100% verde (3º tick seguido)
- **Estado**: credencial restaurada segue funcionando — watchdog **exit 0** com `SMTP OK · IMAP OK`. Caixa verificada: **0 respostas pendentes** (`replies_pending.json` = `[]`).

### Números do estado real (pós-tick 18:31)
- **Leads totais:** **174** — ativos: **132** (bounces cache: 42).
- **Sync Formspree:** ✅ OK — 0 notificações novas.
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos já tentados).
- **Watchdog:** ✅ **exit 0** — nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup:** ✅ 0 follow-ups processados.
- **send_sequence:** ✅ sequência processada (sem envios novos).
- **check-replies:** ✅ 0 respostas aguardando.
- **enviar_lote --status:** 134 empresas / 123 contatados / 29 bounce / **0 pendentes**.
- **Dashboard:** **148 apresentações / 124 entregues / 24 bounces (16%) / 12 respostas / taxa 8,1%** (meta >3%) — funil convertendo acima da meta.
...
### Problemas encontrados e correções
- **Nenhum problema novo.** Sistema 100% operacional pelo 3º tick após o fim do bloqueio de 26 ticks. Pendências estruturais mantidas: fila de prospecção em 0 pendentes (gargalo do Prospector — precisa repor 5-8/dia com MX validado) e Netlify sem créditos (403, GitHub Pages segue como único no ar).

### Leads quentes (destaque — ação humana)
1. **Savegnago Supermercados (~100 lojas) — #1:** **PRAZO 10/09 VENCEU HOJE → toque no WhatsApp (11) 96785-9631 (ação humana URGENTE, independente do bot).**
2. **Frango Assado / Grupo IMC — #2:** **PRAZO 10/09 VENCEU HOJE → toque no WhatsApp (11) 96785-9631.**
3. **Confiança Supermercados (id 171) — 🎯 mais quente do dia:** SAC (protocolo 234776, Larissa Arruda) indicou **social@confianca.com.br** — proposta enviada ao setor social; rede = nicho que mais converte. Acompanhar retorno; toque WhatsApp se silêncio em 3 dias.
4. **Pastificio Selmi S/A — #3:** compras@selmi.com.br. **Prazo: 11/09 (amanhã) → toque leve se silêncio.**
5. **Queijos Itupeva** — respondeu 16:13 (aguardando volume mensal + tipo), nicho vencidos >40% gordura.
6. **Rede Bom Lugar, Tenda Atacado, Roldão** — aguardando; **Sumerbol (Ivone), GoodBom (Laura), Rede Boa (#23915), Arcor/Bagley, Sanofi (02995121)** — em aberto.

### Nota
Tick tranquilo e estável: nada pendente, nada atrasado, caixa limpa. O foco continua no **fechamento humano** — os prazos de WhatsApp de Savegnago e Grupo IMC venceram hoje, Selmi vence amanhã; o bot já fez a parte dele (apresentação + follow-ups + respostas em dia). Fila de prospecção segue em 0 pendentes — reposição do Prospector é o único gargalo estrutural restante.

---

## 2026-09-10 (quinta) — ATENDENTE (tick 18:10)

### ✅ CREDENCIAL RESTAURADA — fim do alerta crítico (26 ticks)
- **Estado**: senha de app do Gmail **restaurada** (config.json atualizado às 16:08). Watchdog agora valida **SMTP OK · IMAP OK** (exit 0) — primeira execução totalmente verde desde 09/09 11:01. Caixa verificada: 0 respostas pendentes (`replies_pending.json` = `[]`).

### Números do estado real (pós-tick 18:10)
- **Leads totais:** **174** — `novo`: 105 | `sequencia`: 27 | `respondido`: 14 | `bounce`: 26 | `encerrado`: 2 — **Ativos: 132** (bounces cache: 42).
- **Sync Formspree:** ✅ OK — 0 notificações novas (cache de bounce 42).
- **corrigir_emails:** ✅ OK — 0 bounces processados (todos já tentados; BRF Itu `sac@brf.com.br` sem email alternativo válido).
- **Watchdog:** ✅ **exit 0** — SMTP OK · IMAP OK; nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup:** ✅ 0 follow-ups processados — **backlog de 11 FP1 ZERADO** (disparados automaticamente quando a credencial voltou).
- **send_sequence:** ✅ sequência processada (sem envios novos pendentes).
- **check-replies:** ✅ 0 respostas aguardando — respostas órfãs do período de bloqueio **já atendidas no tick 16:13** (Confiança Supermercados + Queijos Itupeva).
- **enviar_lote --status:** 134 empresas / 123 contatados / 29 bounce / **0 pendentes** (Mambo, BRF Itu, Piraquê enviados hoje após a restauração; BRF Itu confirmado bounce).
- **Dashboard:** **148 apresentações / 124 entregues / 24 bounces (16%) / 12 respostas / taxa 8,1%** (meta >3%) — funil segue convertendo acima da meta.

### Ações do tick
- Nenhum email enviado neste tick (nada pendente — fila zerada, follow-ups em dia, caixa limpa).
- Nenhum pedido de relatório ESG/certificado de impacto — sem geração de PDF.

### Problemas encontrados e correções
1. **🚨 CRÍTICO RESOLVIDO — senha de app restaurada.** O bloqueio de 26 ticks (09/09 11:01 → 10/09 16:08) acabou: envios, caixa, sync e follow-ups voltaram a funcionar. O watchdog testando SMTP/IMAP em cada execução (melhoria do Melhorador 09/09) funcionou como alarme durante todo o período.
2. **Backlog limpo automaticamente:** 11 FP1 atrasados + 3 pendentes da fila (Mambo, BRF Itu, Piraquê) enviados assim que a credencial voltou; BRF Itu deu bounce (sem alternativo).
3. **Respostas órfãs recuperadas:** Confiança Supermercados e Queijos Itupeva (chegaram durante o bloqueio) processadas no tick 16:13 — nenhuma resposta perdida.
4. Pendências estruturais mantidas: Netlify sem créditos (403) — GitHub Pages segue como único no ar; fila de prospecção com 0 pendentes (gargalo do Prospector).

### Leads quentes (destaque — ação humana)
1. **Confiança Supermercados (id 171) — NOVO QUENTE 🎯:** SAC respondeu hoje (protocolo 234776, Larissa Arruda) indicando **social@confianca.com.br** — proposta enviada ao setor social. Rede de supermercados = **nicho que mais converte** (4 dos 7 respondidos reais). Próximo passo: acompanhar retorno do setor social; toque no WhatsApp (11) 96785-9631 se silêncio em 3 dias.
2. **Queijos Itupeva (id 160) — NOVO:** laticínio artesanal (Itupeva) respondeu 16:13 — nicho de vencidos >40% gordura (queijos = separação difícil, piso R$ 0,50/kg); resposta enviada pedindo volume mensal + tipo + WhatsApp.
3. **Savegnago Supermercados (~100 lojas) — #1:** **PRAZO 10/09 VENCEU HOJE → toque no WhatsApp (11) 96785-9631 (ação humana urgente).**
4. **Frango Assado / Grupo IMC — #2:** **PRAZO 10/09 VENCEU HOJE → toque no WhatsApp (11) 96785-9631.**
5. **Pastificio Selmi S/A (id 60) — #3:** compras@selmi.com.br (proposta 08/09). **Prazo: 11/09 (amanhã) → toque leve se silêncio.**
6. **Rede Bom Lugar (id 132), Tenda Atacado (id 133), Roldão Atacadista (id 134)** — aguardando retorno.
7. **Sumerbol (id 21)** — Ivone Franca; **GoodBom (id 51)** — Laura (19) 3828-9798; **Rede Boa (id 46)** — ticket #23915; **Arcor/Bagley (id 14)**; **Sanofi Medley (id 91)** — protocolo 02995121.

### Nota
Tick histórico: o bloqueio de credencial de 26 ticks (o maior incidente do ecossistema) **foi resolvido** e o pipeline voltou 100% verde — envios, follow-ups, caixa e sync funcionando. Duas respostas órfãs recuperadas e atendidas (Confiança Supermercados é o lead mais quente do dia: rede + canal comercial indicado pelo próprio SAC). **Ação humana prioritária: toque no WhatsApp de Savegnago e Grupo IMC (prazos venceram hoje) e de Selmi amanhã.** Fila de prospecção segue em 0 pendentes — Prospector precisa repor.
...
- **Estado**: senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, reconfirmado às 18:01 neste tick (login direto IMAP, com e sem espaços). Revogada entre 11:01 e 11:33 de 09/09. **Caixa NÃO verificada desde 11:01** — mais de 7h de respostas potencialmente órfãs.
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 14º tick seguido**. Nenhum envio é possível. Respostas de leads em maturação (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar) podem estar na caixa sem atendimento.
- **AÇÃO NECESSÁRIA (humana, urgente)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app` → rodar `python sync_formspree.py && python bot_oleo.py check-replies`.
- **Offline segue 100% verde**: fila ✅ (0 pendentes — 131/120/28), sequência ✅ (nada a enviar), follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).
...
### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente, 14º tick) — senha de app revogada.** Único bloqueio real; correção 100% manual (nova senha de app no Google). O watchdog com teste SMTP/IMAP segue alarmando no primeiro tick da queda — funcionou como projetado.
2. Nenhum problema novo além do bloqueio — todo o restante do pipeline saudável offline (0 atrasados, 0 pendentes, 0 bounces novos).
...
---
## 2026-09-09 (quarta) — ATENDENTE (tick 17:31)

### 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (13º tick consecutivo)
- **Estado**: senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, reconfirmado às 17:31 neste tick (login direto IMAP, com e sem espaços). Revogada entre 11:01 e 11:33 de 09/09. **Caixa NÃO verificada desde 11:01** — mais de 6h30 de respostas potencialmente órfãs.
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 13º tick seguido**. Nenhum envio é possível. Respostas de leads em maturação (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar) podem estar na caixa sem atendimento.
- **AÇÃO NECESSÁRIA (humana, urgente)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app` → rodar `python sync_formspree.py && python bot_oleo.py check-replies`.
- **Offline segue 100% verde**: fila ✅ (0 pendentes — 131/120/28), sequência ✅ (nada a enviar), follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).

### Números do estado real (pós-tick 17:31)
- **Leads totais:** **170** — `novo`: 104 | `sequencia`: 27 | `respondido`: 12 | `bounce`: 25 | `encerrado`: 2 — **Ativos: 131** (pipeline inalterado).
- **Sync Formspree:** ❌ **FALHOU** (credencial) — cache de bounce permanece **41**; 0 notificações novas.
- **corrigir_emails:** ✅ OK offline — 0 novas correções (todos os 41 bounces com tentativa registrada).
- **Watchdog:** ❌ **exit 1** com os 2 alertas esperados (🔴 SMTP 535 + 🔴 IMAP AUTHENTICATIONFAILED). `--skip-email` → ✅ exit 0: nenhum follow-up atrasado, bounce sem correção ou lead parado.
...
### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente, 13º tick) — senha de app revogada.** Único bloqueio real; correção 100% manual (nova senha de app no Google). O watchdog com teste SMTP/IMAP segue alarmando no primeiro tick da queda — funcionou como projetado.
2. Nenhum problema novo além do bloqueio — todo o restante do pipeline saudável offline (0 atrasados, 0 pendentes, 0 bounces novos).
...
---
## 2026-09-09 (quarta) — ATENDENTE (tick 17:02)

### 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (12º tick consecutivo)
- **Estado**: senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, reconfirmado às 17:02 neste tick (login direto IMAP, com e sem espaços). Revogada entre 11:01 e 11:33 de 09/09. **Caixa NÃO verificada desde 11:01** — mais de 6h de respostas potencialmente órfãs.
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 12º tick seguido**. Nenhum envio é possível. Respostas de leads em maturação (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar) podem estar na caixa sem atendimento.
- **AÇÃO NECESSÁRIA (humana, urgente)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app` → rodar `python sync_formspree.py && python bot_oleo.py check-replies`.
- **Offline segue 100% verde**: fila ✅ (0 pendentes — 131/120/28), sequência ✅ (nada a enviar), follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).

### Números do estado real (pós-tick 17:02)
- **Leads totais:** **170** — `novo`: 104 | `sequencia`: 27 | `respondido`: 12 | `bounce`: 25 | `encerrado`: 2 — **Ativos: 131** (pipeline inalterado).
- **Sync Formspree:** ❌ **FALHOU** (credencial) — cache de bounce permanece **41**; 0 notificações novas.
- **corrigir_emails:** ✅ OK offline — 0 novas correções (todos os 41 bounces com tentativa registrada).
- **Watchdog:** ❌ **exit 1** com os 2 alertas esperados (🔴 SMTP 535 + 🔴 IMAP AUTHENTICATIONFAILED). `--skip-email` → ✅ exit 0: nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup:** 0 processados (nada atrasado). **send_sequence:** nada a enviar.
- **check-replies:** ❌ **FALHOU** (credencial) — `replies_pending.json` vazio (`[]`; última checagem real: 11:01).
- **enviar_lote --status:** 131 empresas / 120 contatados / 28 bounce / **0 pendentes**.
- **Dashboard:** 145 apresentações / 122 entregues / 23 bounces (16%) / **11 respostas** / taxa **7,6%** (meta >3%).
- **Emails enviados neste tick:** 0. **Relatório ESG:** nenhum pedido (caixa inacessível).
...
### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente, 12º tick) — senha de app revogada.** Único bloqueio real; correção 100% manual (nova senha de app no Google). O watchdog com teste SMTP/IMAP segue alarmando no primeiro tick da queda — funcionou como projetado.
2. Nenhum problema novo além do bloqueio — todo o restante do pipeline saudável offline (0 atrasados, 0 pendentes, 0 bounces novos).
...
## 2026-09-09 (quarta) — ATENDENTE (tick 16:31)

### 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (11º tick consecutivo)
- **Estado**: senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, reconfirmado às 16:31 neste tick (login direto IMAP, com e sem espaços). Revogada entre 11:01 e 11:33 de 09/09. **Caixa NÃO verificada desde 11:01** — mais de 5h30 de respostas potencialmente órfãs.
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 11º tick seguido**. Nenhum envio é possível. Respostas de leads em maturação (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar) podem estar na caixa sem atendimento.
- **AÇÃO NECESSÁRIA (humana, urgente)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app` → rodar `python sync_formspree.py && python bot_oleo.py check-replies`.
- **Offline segue 100% verde**: fila ✅ (0 pendentes — 131/120/28), sequência ✅ (nada a enviar), follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).
...
### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente, 11º tick) — senha de app revogada.** Único bloqueio real; correção 100% manual (nova senha de app no Google). O watchdog com teste SMTP/IMAP segue alarmando no primeiro tick da queda — funcionou como projetado.
2. Nenhum problema novo além do bloqueio — todo o restante do pipeline saudável offline (0 atrasados, 0 pendentes, 0 bounces novos).
...
## 2026-09-09 (quarta) — ATENDENTE (tick 16:01)

### 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (10º tick consecutivo)
- **Estado**: senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, reconfirmado às 16:00 neste tick (login direto IMAP, com e sem espaços). Revogada entre 11:01 e 11:33 de 09/09. **Caixa NÃO verificada desde 11:01** — mais de 5h de respostas potencialmente órfãs.
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 10º tick seguido**. Nenhum envio é possível. Respostas de leads em maturação (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar) podem estar na caixa sem atendimento.
- **AÇÃO NECESSÁRIA (humana, urgente)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app` → rodar `python sync_formspree.py && python bot_oleo.py check-replies`.
- **Offline segue 100% verde**: fila ✅ (0 pendentes — 131/120/28), sequência ✅ (nada a enviar), follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).
...
- **enviar_lote --status:** 131 empresas / 120 contatados / 28 bounce / **0 pendentes**.
- **Dashboard:** 145 apresentações / 122 entregues / 23 bounces (16%) / **11 respostas** / taxa **7,6%** (meta >3%).
- **Emails enviados neste tick:** 0. **Relatório ESG:** nenhum pedido.

### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente, 10º tick) — senha de app revogada.** Único bloqueio real; correção 100% manual (nova senha de app no Google). O watchdog com teste SMTP/IMAP segue alarmando no primeiro tick da queda — funcionou como projetado.
2. Nenhum problema novo além do bloqueio — todo o restante do pipeline saudável offline.
...
3. **Frango Assado / Grupo IMC — #3:** felix.costa@grupoimc.com.br (07/09). **PRAZO: 10/09 (AMANHÃ) → toque no WhatsApp.**
4. **Tauste (156)** — compras CD Valinhos (08/09 14:33); se responder, tratar como #1. **Enxuto (155) + Chimar (157)** — aguardando.
5. **Grupo Lanchero** (vencidos >40% + descaracterização), **nicho encapsulados/farmacêuticas** (Arese, Infinity Pharma, Veridi, Larimar, Maila, Farmaviva, Audacci, GDS USA), **Andorinha Salto (147)** — aguardando retorno (caixa sem verificação desde 11:01).

---

## 2026-09-09 (quarta) — ATENDENTE (tick 15:32)

### 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (9º tick consecutivo)
- **Estado**: senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, reconfirmado às 15:31 neste tick (login direto IMAP, com e sem espaços). Revogada entre 11:01 e 11:33 de 09/09. **Caixa NÃO verificada desde 11:01** — mais de 4h30 de respostas potencialmente órfãs.
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 9º tick seguido**. Nenhum envio é possível. Respostas de leads em maturação (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar) podem estar na caixa sem atendimento.
- **AÇÃO NECESSÁRIA (humana, urgente)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app` → rodar `python sync_formspree.py && python bot_oleo.py check-replies`.
- **Offline segue 100% verde**: fila ✅ (0 pendentes — 131/120/28), sequência ✅ (nada a enviar), follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).
...
- **Sync Formspree:** ❌ **FALHOU** (credencial) — cache de bounce permanece **41**; 0 notificações novas.
- **corrigir_emails:** ✅ OK offline — 0 novas correções (todos os 41 bounces com tentativa registrada).
- **Watchdog:** ❌ **exit 1** com os 2 alertas esperados (🔴 SMTP 535 + 🔴 IMAP AUTHENTICATIONFAILED). Offline saudável: nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup:** 0 processados (nada atrasado). **send_sequence:** nada a enviar.
- **check-replies:** ❌ **FALHOU** (credencial) — `replies_pending.json` vazio (`[]`; última checagem real: 11:01).
- **enviar_lote --status:** 131 empresas / 120 contatados / 28 bounce / **0 pendentes**.
- **Dashboard:** 145 apresentações / 122 entregues / 23 bounces (16%) / **11 respostas** / taxa **7,6%** (meta >3%).
...
### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente, 9º tick) — senha de app revogada.** Único bloqueio real; correção 100% manual (nova senha de app no Google). O watchdog com teste SMTP/IMAP (melhoria do tick 14:10) segue alarmando no primeiro tick da queda — funcionou como projetado.
2. Nenhum problema novo além do bloqueio — todo o restante do pipeline saudável offline.
...
## 2026-09-09 (quarta) — ATENDENTE (tick 15:03)

### 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (4º tick consecutivo)
- **Estado**: senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, reconfirmado às 15:02 neste tick (login direto IMAP, com e sem espaços). **Caixa NÃO verificada desde 11:01** — mais de 4h de respostas potencialmente órfãs.
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 4º tick seguido**. Nenhum envio é possível. Respostas de leads em maturação (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar) podem estar na caixa sem atendimento.
- **AÇÃO NECESSÁRIA (humana, urgente)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app` → rodar `python sync_formspree.py && python bot_oleo.py check-replies`.
- **Offline segue 100% verde**: watchdog `--skip-email` ✅ exit 0 (170 leads / 131 ativos / 41 bounces), fila ✅ (0 pendentes), sequência ✅, follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).

### Números do estado real (pós-tick 15:03)
- **Leads totais:** **170** — `novo`: 104 | `sequencia`: 27 | `respondido`: 12 | `bounce`: 25 | `encerrado`: 2 — **Ativos: 131** (pipeline inalterado).
- **Sync Formspree:** ❌ **FALHOU** (credencial) — cache de bounce permanece **41**; 0 notificações novas.
- **corrigir_emails:** ✅ OK offline — 0 novas correções (todos os 41 bounces com tentativa registrada).
- **Watchdog:** ❌ **exit 1** com os 2 alertas esperados (🔴 SMTP 535 + 🔴 IMAP AUTHENTICATIONFAILED). `--skip-email` → ✅ exit 0: nenhum follow-up atrasado, bounce sem correção ou lead parado.
...
### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente, 4º tick) — senha de app revogada.** Único bloqueio real; correção 100% manual (nova senha de app no Google). O watchdog com teste SMTP/IMAP (melhoria do tick 14:10) agora **alarma no primeiro tick da queda** — funcionou como projetado: exit 1 imediato com diagnóstico claro.
2. Nenhum problema novo além do bloqueio — todo o restante do pipeline saudável offline.
...
- **Estado**: a senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, reconfirmado às 12:32 neste tick (com e sem espaços). Não é transitória: foi revogada/trocada entre 11:01 e 11:33 de 09/09 (às 11:01 o login IMAP ainda funcionava). **Quase 2h de caixa sem verificação.**
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 3º tick seguido**. Caixa **NÃO verificada desde 11:01** — respostas de leads em maturação (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar, Andorinha) podem estar órfãs e sem atendimento.
- **AÇÃO NECESSÁRIA (humana)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app`. Depois rodar `python sync_formspree.py && python bot_oleo.py check-replies` para zerar o risco de resposta órfã. **Amanhã (10/09) vencem os prazos de toque no WhatsApp de Savegnago e Grupo IMC — ação humana prioritária independente da credencial.**
- **Offline segue 100% verde**: watchdog ✅ (exit 0), fila ✅ (0 pendentes), sequência ✅, follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).

### Números do estado real (pós-tick 12:33)
- **Leads totais:** **170** — `novo`: 104 | `sequencia`: 27 | `respondido`: 12 | `bounce`: 25 | `encerrado`: 2 — **Ativos: 131** (pipeline inalterado, contagem direta no leads.csv).
- **Sync Formspree:** **FALHOU** (credencial) — cache de bounce permanece **41**; 0 notificações novas.
- **corrigir_emails:** ✅ OK offline — 0 novas correções (todos os 41 bounces com tentativa registrada).
- **Watchdog:** ✅ **exit 0** — 170 leads / 131 ativos / 41 bounces; 0 follow-ups atrasados, 0 bounces sem correção, 0 leads parados.
- **prospecao_followup:** 0 processados. **send_sequence:** nada a enviar.
- **check-replies:** ❌ **FALHOU** (credencial) — `replies_pending.json` vazio (última checagem real: 11:01).
- **enviar_lote --status:** 131 empresas / 120 contatados / 28 bounce / **0 pendentes**.
- **Emails enviados neste tick:** 0. **Relatório ESG:** nenhum pedido.

### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente, 3º tick) — senha de app revogada.** Único bloqueio real do sistema; correção 100% manual (gerar nova senha de app no Google). Enquanto isso, leads que responderem entre 11:01 e a correção ficam sem atendimento — prioridade nº 1. Sugestão já registrada: watchdog deveria testar SMTP/IMAP no início de cada tick e alarmar na hora.
2. Nenhum problema novo além do bloqueio de credencial — todo o resto do pipeline segue saudável offline.
...
3. **Frango Assado / Grupo IMC — #3:** felix.costa@grupoimc.com.br (07/09). **PRAZO: 10/09 (amanhã) → toque no WhatsApp.**
4. **Tauste (156)** — compras CD Valinhos (08/09 14:33); se responder, tratar como #1. **Enxuto (155) + Chimar (157)** — aguardando.
5. **Grupo Lanchero** (vencidos >40% + descaracterização), **nicho encapsulados/farmacêuticas**, **Andorinha Salto (147)** — aguardando retorno (caixa não verificada desde 11:01).

---

## 2026-09-09 (quarta) — ATENDENTE (tick 12:05)

### 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (2º tick consecutivo)
- **Estado**: a senha de app (`masteroleo.eco@gmail.com` em `bot/config.json`) **continua rejeitada** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, confirmado de novo neste tick (com e sem espaços, 3 retries). Não é transitória: foi revogada/trocada entre 11:01 e 11:33 de 09/09 (às 11:01 o login IMAP ainda funcionava).
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM pelo 2º tick seguido**. Caixa **NÃO verificada** desde 11:01 — respostas de leads que chegarem nesse intervalo ficam sem atendimento (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar estão no prazo de maturação).
- **AÇÃO NECESSÁRIA (humana)**: gerar nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app`. Depois rodar `python sync_formspree.py && python bot_oleo.py check-replies`.
- **Offline segue 100% verde**: watchdog ✅ (exit 0), fila ✅ (0 pendentes), sequência ✅, follow-ups ✅ (0 atrasados), correção de bounces ✅.

### Números do estado real (pós-tick 12:05)
- **Leads totais:** **170** — `novo`: 104 | `sequencia`: 27 | `respondido`: 12 | `bounce`: 25 | `encerrado`: 2 — **Ativos: 131** (pipeline inalterado, contagem direta no leads.csv).
- **Sync Formspree:** **FALHOU** (credencial) — cache de bounce permanece **41**; 0 notificações novas.
- **corrigir_emails:** ✅ OK offline — 0 novas correções (23 bounces de prospecção, todos com tentativa registrada).
- **Watchdog:** ✅ **exit 0** — 170 leads / 131 ativos / 41 bounces; 0 follow-ups atrasados, 0 bounces sem correção, 0 leads parados.
- **prospecao_followup:** 0 processados. **send_sequence:** nada a enviar.
- **check-replies:** ❌ **FALHOU** (credencial) — `replies_pending.json` vazio (última checagem real: 11:01).
- **enviar_lote --status:** 131 empresas / 120 contatados / 28 bounce / **0 pendentes**.
...
### Problemas encontrados e correções
1. **🚨 CRÍTICO (persistente) — senha de app revogada.** Único bloqueio real do sistema; correção 100% manual (gerar nova senha de app no Google). Enquanto isso, leads que responderem entre 11:01 e a correção ficam sem atendimento — prioridade nº 1.
2. Sugestão já registrada para o Melhorador: watchdog deveria testar SMTP/IMAP no início de cada tick e alarmar na hora (hoje a falha só aparece quando um script conecta).
...
3. **Frango Assado / Grupo IMC — #3:** felix.costa@grupoimc.com.br (07/09). **PRAZO: 10/09 (amanhã) → toque no WhatsApp.**
4. **Tauste (156)** — compras CD Valinhos (08/09 14:33); se responder, tratar como #1. **Enxuto (155) + Chimar (157)** — aguardando.
5. **Grupo Lanchero** (vencidos >40% + descaracterização), **nicho encapsulados/farmacêuticas**, **Andorinha Salto (147)** — aguardando retorno.

## 2026-09-09 (quarta) — ATENDENTE (tick 11:33)

### 🚨 ALERTA CRÍTICO — senha de app do Gmail REVOGADA
- **O que aconteceu**: a senha de app do Gmail (`masteroleo.eco@gmail.com`) em `bot/config.json` **funcionava às 11:01** (bounces.json regravado via login IMAP OK) e **passou a ser rejeitada às 11:33** — `AUTHENTICATIONFAILED` no IMAP e `535 BadCredentials` no SMTP, com e sem espaços, em 4 tentativas independentes.
- **Impacto**: `sync_formspree.py` e `bot_oleo.py check-replies` **FALHARAM** neste tick. A caixa **NÃO foi verificada** — respostas de leads que chegarem entre 11:01 e a correção ficam sem atendimento (risco real: Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar estão no prazo de maturação).
- **AÇÃO NECESSÁRIA (humana)**: gerar nova senha de app em https://myaccount.google.com/apppasswords e atualizar `bot/config.json` → `email.senha_app`. Nenhum script consegue corrigir isso sozinho.
- **O que segue funcionando offline**: watchdog ✅ (exit 0), fila de prospecção ✅ (0 pendentes), sequência ✅ (nada a enviar), follow-ups ✅ (0 atrasados), correção de bounces ✅ (nada pendente).

### Números do dia (estado real pós-tick)
- **Sync Formspree:** **FALHOU** (IMAP AUTHENTICATIONFAILED) — cache de bounce permanece **41**; 0 notificações novas não processadas.
- **corrigir_emails:** não executado (sync falhou no `&&`). `--status` offline OK: 23 bounces de prospecção, todos os 41 do cache com tentativa registrada — nada pendente.
- **Leads totais:** **170** — `novo`: 104 | `sequencia`: 27 | `respondido`: 12 | `bounce`: 25 | `encerrado`: 2 — **Ativos: 131** (pipeline inalterado, contagem direta no leads.csv).
- **Watchdog:** ✅ saudável (**exit 0**) — 170 leads, 131 ativos, 41 bounces; nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup.py:** 0 follow-ups processados (nada atrasado).
- **send_sequence:** sequência processada, sem envios novos (nada pendente).
- **check-replies:** **FALHOU** (IMAP AUTHENTICATIONFAILED) — **CAIXA NÃO VERIFICADA**; `replies_pending.json` segue vazio (última checagem real: 11:01).
- **enviar_lote --status:** fila com **131 empresas / 120 contatados / 28 bounce / 0 pendentes** — nada a enviar.
- **Emails enviados neste tick:** **0** — nada pendente para envio (credencial bloqueada impediria envio de qualquer forma).

### Ações do tick
- Nenhum email enviado/recebido; tentativa de sync e de checagem de caixa bloqueadas pela credencial revogada.
- Nenhum pedido de relatório ESG/certificado de impacto — sem geração de PDF.
- Diagnóstico de credencial executado em 4 tentativas (IMAP/SMTP, com/sem espaços) — falha consistente, não transitória.

### Problemas encontrados e correções
1. **🚨 CRÍTICO — senha de app do Gmail REVOGADA/TROCADA entre 11:01 e 11:33 de 09/09.** IMAP e SMTP rejeitam (`AUTHENTICATIONFAILED` / `535 BadCredentials`). Bloqueia sync_formspree, check-replies, scan_inbox e qualquer envio. **Correção depende de ação humana**: nova senha de app em https://myaccount.google.com/apppasswords → atualizar `bot/config.json` → `email.senha_app`. Após corrigir, rodar `python sync_formspree.py && python bot_oleo.py check-replies` para zerar o risco de respostas sem atendimento.
2. Pendências estruturais mantidas (não são deste tick): Netlify sem créditos (403); inbound via site fraco; fila de prospecção com **0 pendentes** — gargalo nº1 do Estrategista.
3. **Sugestão de melhoria (para o Melhorador)**: watchdog deveria testar SMTP/IMAP no início de cada tick e emitir alerta imediato se a credencial cair — hoje a falha só aparece quando um script tenta conectar.
...
6. **Grupo Lanchero Alimentos:** indústria de maioneses/molhos — encaixe direto com **resíduos vencidos >40% gordura + descaracterização** (argumento de passivo do CNPJ). Enviado 08/09 10:20.
7. **Nicho máquinas/encapsulados (aguardando resposta):** Arese Farmaceutica (sac@arese.com.br), Infinity Pharma, Veridi Nutrition, Larimar, Maila, Farmaviva, Audacci, GDS USA.
8. **Andorinha Hiper Center (147) + Paulistão Atacadista (149) + Dalben (148) + Amigão (151) + São Roque (130) + Confiança (131)** — monitorar replies.

### Para o Estrategista
- **Prioridade nº 1 do dia: restaurar a credencial de email** — sem ela, o funil inteiro (sync, respostas, envios) fica parado. Depois, rodar check-replies para garantir que nenhuma resposta ficou órfã.
...
- **corrigir_emails:** 0 bounces processados — todos os 40 do cache já com tentativa registrada (22 "já tentado"); nada novo.
- **Leads totais:** **160** — `novo`: 95 | `sequencia`: 27 | `respondido`: 12 | `bounce`: 24 | `encerrado`: 2 — **Ativos: 122**. Pipeline **inalterado** desde o tick 14:34 (nenhum lead novo desde os supermercados das 14:33).
- **Watchdog:** ✅ saudável (**exit 0**) — 160 leads, 122 ativos, 40 bounces no cache; nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup.py:** 0 follow-ups processados (nada atrasado).
- **send_sequence:** sequência processada, sem envios novos (leads de prospecção têm fluxo próprio via enviar_lote).
- **check-replies:** **0 pendentes** (`replies_pending.json` vazio — confirmado `[]`); **scan_inbox (contingência):** 0 não lidas/recentes na INBOX — caixa confirmada em dobro, nada novo desde a Selmi (10:08).
- **enviar_lote --status:** fila com **121 empresas / 110 contatados / 27 bounce / 0 pendentes** — nada a enviar.
- **Emails enviados neste tick:** **0** — tick de rotina, nada pendente.

### Ações do tick
- Nenhum email enviado/recebido — dia de aguardar retornos dos canais comerciais abertos (Selmi, Savegnago e Grupo IMC).
- Nenhum pedido de relatório ESG/certificado de impacto — sem geração de PDF.
- Caixa verificada em dobro (check-replies + scan_inbox) — zero respostas perdidas.

### Problemas encontrados e correções
1. **Nenhum problema operacional novo.** Sync, watchdog, correções, follow-ups, sequência e caixa 100% verdes.
2. Pendências estruturais mantidas (não são deste tick): Netlify sem créditos (403); inbound via site fraco; fila de prospecção com **0 pendentes** — gargalo nº1 do Estrategista (Prospector precisa repor 5-8/dia).
...
4. **Tauste Supermercados (id 156):** apresentação enviada 14:33 para `custo@tauste.com.br` (email de COMPRAS do CD de Valinhos). Rede com 3 lojas em Sorocaba + Campinas + Jundiaí. **Se responder: tratar como lead #1 ao lado de Savegnago.**
5. **Enxuto (155) + Chimar (157):** apresentações enviadas 14:33 (template V4 supermercados) — Chimar é a matriz de Mairinque, ~20 km de Salto.
6. **Grupo Lanchero Alimentos:** indústria de maioneses/molhos — encaixe direto com **resíduos vencidos >40% gordura + descaracterização** (argumento de passivo do CNPJ). Enviado 10:20; se responder: pedir volume + tipo e oferecer coleta-teste + relatório ESG.
7. **Nicho máquinas/encapsulados (aguardando resposta):** Arese Farmaceutica (sac@arese.com.br), Infinity Pharma, Veridi Nutrition, Larimar, Maila, Farmaviva, Audacci, GDS USA — se responderem: pedir volume + tipo + coleta-teste.

### Para o Estrategista
- Taxa de resposta segue **~8,3% (11 respostas / 132 apresentações)** — acima da meta, funil converte. O gargalo real é o pipeline: fila com **0 pendentes** de novo.
- Foco da semana (meta 14/09: 1 coleta-teste/contrato): **Savegnago, Sumerbol, GoodBom, Selmi** + monitorar Tauste.

---

## 2026-09-08 (terça) — ATENDENTE (tick 17:31)
...
4. **Tauste Supermercados (id 156):** apresentação enviada 14:33 para `custo@tauste.com.br` (email de COMPRAS do CD de Valinhos). Rede com 3 lojas em Sorocaba + Campinas + Jundiaí. **Se responder: tratar como lead #1 ao lado de Savegnago.**
5. **Enxuto (155) + Chimar (157):** apresentações enviadas 14:33 (template V4 supermercados) — Chimar é a matriz de Mairinque, ~20 km de Salto.
6. **Nicho máquinas/encapsulados (aguardando resposta):** Arese Farmaceutica (sac@arese.com.br), Infinity Pharma, Veridi Nutrition, Larimar, Maila, Farmaviva, Audacci, GDS USA — se responderem: pedir volume + tipo + coleta-teste.
...
## 2026-09-08 (terça) — ATENDENTE (tick 17:01)

### Números do dia (estado real pós-tick)
- **Sync Formspree:** **40** emails no cache de bounce; **0** notificações novas (nenhum lead novo via site).
- **corrigir_emails:** 0 bounces processados — todos os 40 do cache já com tentativa registrada (22 "já tentado"); nada novo.
- **Leads totais:** **160** — `novo`: 95 | `sequencia`: 27 | `respondido`: 12 | `bounce`: 24 | `encerrado`: 2 — **Ativos: 122** (+3 vs 12:00: Enxuto, Tauste, Chimar).
- **Watchdog:** ✅ saudável (**exit 0**) — 160 leads, 122 ativos, 40 bounces no cache; nenhum follow-up atrasado, bounce sem correção ou lead parado.
- **prospecao_followup.py:** 0 follow-ups processados (nada atrasado).
- **send_sequence:** sequência processada, sem envios novos (leads de prospecção têm fluxo próprio via enviar_lote).
- **check-replies:** **0 pendentes** (`replies_pending.json` vazio — confirmado `[]`) — nenhuma resposta nova desde a Selmi (10:08).
- **enviar_lote --status:** fila com **121 empresas / 110 contatados / 27 bounce / 0 pendentes**.
- **Emails enviados neste tick:** **0** — tick de rotina, nada pendente.

### Ações do tick
- Nenhum email enviado/recebido — dia de aguardar retornos dos canais comerciais abertos (Selmi, Savegnago e Grupo IMC).
- Nenhum pedido de relatório ESG/certificado de impacto — sem geração de PDF.
- **Pendência do Melhorador 14:10 ENCERRADA:** os 3 emails de supermercado (Enxuto, Tauste, Chimar) **já foram enviados às 14:33** (campo `apresentacao_em` preenchido no leads.csv; fila saltou de 107 → 110 contatados). O pipeline de prospecção seguiu sem intervenção.

### Problemas encontrados e correções
1. **Nenhum problema operacional novo.** Sync, watchdog, correções, follow-ups, sequência e caixa 100% verdes.
2. Correções estruturais anteriores seguem em vigor: `corrigir_nicho_manual.py` (Arese → `sac@arese.com.br`, HRT `nao_encontrado`) e fila reabastecida com 3 redes (Enxuto/Tauste/Chimar).
3. Pendências estruturais mantidas (não são deste tick): Netlify sem créditos (403); inbound via site fraco; fila de prospecção novamente com **0 pendentes** — gargalo nº1 do Estrategista (Prospector precisa repor 5-8/dia).

### Leads quentes (para ação humana — destaque)
1. **Pastificio Selmi S/A (id 60) — #1:** canal comercial ABERTO via SAC (proposta a **compras@selmi.com.br** 08/09 10:08). **PRAZO: sem retorno até 11/09 → toque leve no WhatsApp (11) 96785-9631.**
2. **Savegnago Supermercados (~100 lojas) — #2:** porta comercial oficial ABERTA (comercial@savegnago.com.br, apresentação 07/09). **PRAZO: sem retorno até 10/09 → toque no WhatsApp.**
3. **Frango Assado / Grupo IMC — #3:** canal comercial ABERTO (felix.costa@grupoimc.com.br, apresentação 07/09). **PRAZO: sem retorno até 10/09 → toque no WhatsApp.**
4. **Tauste Supermercados (id 156) — NOVO e mais promissor do lote:** apresentação enviada 14:33 para `custo@tauste.com.br` (email de COMPRAS do CD de Valinhos). Rede com 3 lojas em Sorocaba + Campinas + Jundiaí. **Se responder: tratar como lead #1 ao lado de Savegnago.**
5. **Enxuto (155) + Chimar (157):** apresentações enviadas 14:33 (template V4 supermercados) — Chimar é a matriz de Mairinque, ~20 km de Salto.
6. **Nicho máquinas/encapsulados (aguardando resposta):** Arese Farmaceutica (sac@arese.com.br), Infinity Pharma, Veridi Nutrition, Larimar, Maila, Farmaviva, Audacci, GDS USA — se responderem: pedir volume + tipo + coleta-teste.
...
## 2026-09-12 (sábado) — ATENDENTE (tick 11:04)

### Resumo do tick

- **Pipeline (snapshot 11:04)**: 181 leads · **15 respondidos** · 26 sequência · **111 novos** · 26 bounce (CSV) / **44 cache** · **3 encerrados** · **ativos: 137**. Funil acima da meta (taxa 8,4-8,6% > meta 3%) pelo 2º mês.
- **Sync**: Formspree OK (0 notificações novas); corrigir_emails OK (0 pendentes de prospecção).
- **Watchdog**: exit 1 no 1º run (1 bounce sem tentativa de correção) → **RESOLVIDO** → exit 0 no 2º run. SMTP OK · IMAP OK.
- **Prospecção/follow-up**: 0 atrasados; sequência processada sem envios novos (os 7 FP2 já foram às 10:01).
- **Caixa**: check-replies 0 pendentes + scan_inbox 0 não lidas — confirmada limpa em dobro. **0 respostas enviadas. Nenhum pedido de relatório ESG.**
- **Fila de prospecção**: 141 empresas / 130 contatados / 29 bounce / **0 pendentes** (gargalo do Prospector).
- **Emails enviados hoje (18)**: 8 FP1 laticínios/queijarias + 3 apresentações (Pelissari, Quata, Marcy) + 7 FP2 refeitórios/rest. industriais.

### Problema encontrado e correção

1. **Bounce da Rede Top (id 142, `ecommerce704@redetop.com.br`) — correção que não foi aplicada no tick anterior:** o tick 10:32 registrou no ecossistema que arquivou o lead como `encerrado`, mas o `leads.csv` **não foi alterado** (lead seguia `bounce`) — o watchdog voltou a acusar. **Corrigido neste tick**: status → `encerrado` no CSV + entrada em `correcoes_emails.json` (`nao_encontrado`, fonte vazia sem dados p/ email alternativo) + backup `leads.csv.bak-20260912-fix`. Watchdog re-rodado → exit 0. **Lição: conferir a persistência no CSV quando o log registrar uma correção.**

### 🏆 Leads quentes (para ação humana — destaque)

1. **Queijos Itupeva (Rafael Galvão) — #1 da semana:** VOLUME REAL informado 11/09 (Frescal 200 kg/mês + manteiga/requeijão/doce 50–100 kg/mês) → proposta enviada com valores por faixa de separação + **descaracterização cortesia**. **Aguardando relação de volumes + coleta-teste — lead mais próximo de contrato** (WhatsApp 11 940333759).
2. **Savegnago (~100 lojas) — toque WhatsApp com prazo 10/09 VENCIDO (URGENTE, ação humana).**
3. **Grupo IMC / Frango Assado — toque WhatsApp com prazo 10/09 VENCIDO.**
4. **Selmi (compras@selmi.com.br) — toque WhatsApp com prazo 11/09 VENCIDO.**
5. **Confiança Supermercados — STAND BY** (recontato em 2-3 meses, protocolo 236485).
...
Rotina 100% verde após 1 correção real de persistência. Caixa limpa em dobro, fila zerada, zero atrasos. Próximas ações humanas: toques de WhatsApp vencidos (Savegnago, Grupo IMC, Selmi) e coleta-teste com Queijos Itupeva.

---

## 2026-09-14 (segunda-feira) — ATENDENTE (tick 08:40)

### Resumo do tick

- **Pipeline (snapshot 08:40)**: 186 leads · 141 ativos · 44 bounces (cache) · 160 apresentações enviadas · 136 entregues · 24 bounces (15%) · **14 respostas** · taxa **8,8%** (meta >3%) — funil acima da meta pelo 2º mês consecutivo.
- **Sync**: Formspree OK (0 notificações novas); corrigir_emails OK (44/44 já tentados, 0 processados).
- **Watchdog**: exit 1 no 1º run — **11 follow-ups ATRASADOS** → **RESOLVIDO** no mesmo tick via prospecao_followup.py (8 FP2 + 2 FP1 em thread + 1 FP1: Conrail, Nutylac, Gomah, Argenzio, São Roque, Tenda, Dalben, Paulistão/Grupo Savegnago, Amigão, Mambo, Piraquê). SMTP OK · IMAP OK.
- **Sequência**: send_sequence OK — **13 FP2 enviados** (frigoríficos: Verdi, Frigorífico 3R, Frisa, Frigorífico do Sul; hotéis: Intercity; farma/suplementos: Infinity Pharma, Veridi, Larimar; cosméticos: Maila, FarmaViva, Audacci, GDS; Arese).
- **Caixa**: check-replies **0 respostas pendentes** (replies_pending.json = []). **0 respostas enviadas. Nenhum pedido de relatório ESG.**
- **Fila de prospecção**: 146 empresas / 135 contatados / 29 bounce / **0 pendentes** (gargalo do Prospector mantido).

### Problemas encontrados e correções

1. **Watchdog exit 1 (11 follow-ups atrasados)** — atraso acumulado desde o último tick (FP1/FP2 de laticínios, atacados e supermercados). **RESOLVIDO**: prospecao_followup.py processou todos os 11 no mesmo tick, zerando os atrasados. Nenhuma resposta de lead ficou sem atendimento (caixa confirmada limpa após o processamento).
2. Pendências estruturais mantidas: fila de prospecção com 0 pendentes (Prospector precisa repor 5-8/dia com MX validado); Netlify sem créditos (403) — GitHub Pages segue como único no ar.

### 🏆 Leads quentes (para ação humana — destaque)

1. **Queijos Itupeva (Rafael Galvão) — #1 da semana:** volume real informado (Frescal 200 kg/mês + derivados 50–100 kg/mês), proposta enviada com descaracterização cortesia. **Aguardando confirmação de coleta-teste — lead mais próximo de contrato** (WhatsApp 11 940333759).
2. **Savegnago (~100 lojas) — toque WhatsApp com prazo 10/09 VENCIDO (URGENTE).** FP2 reenviado hoje ao Paulistão Atacadista (bandeira do Grupo Savegnago) — atenção para não duplicar insistência se o comercial responder.
3. **Grupo IMC / Frango Assado — toque WhatsApp com prazo 10/09 VENCIDO.**
4. **Selmi (compras@selmi.com.br) — toque WhatsApp com prazo 11/09 VENCIDO.**
5. **FP2 de supermercados enviado hoje (nicho que mais converte):** Dalben, Amigão, Tenda Atacado, Paulistão (Grupo Savegnago) — monitorar retornos dos canais comerciais.
6. **Andorinha Hiper Center (SALTO)** — loja na cidade-base; se responder, priorizar coleta-teste na unidade de Salto.
