# Insights — Master Óleo · 09/09/2026 (tick 18:01)

## 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (14º tick consecutivo)
- **Estado**: credencial de `masteroleo.eco@gmail.com` (bot/config.json) **continua rejeitada** — `AUTHENTICATIONFAILED` IMAP / `535 BadCredentials` SMTP, reconfirmado neste tick (18:01, testado com e sem espaços). Revogada entre 11:01 e 11:33 de 09/09.
- **Impacto**: `sync_formspree.py` e `check-replies` **FALHARAM** pelo 14º tick. **Caixa NÃO verificada desde 11:01** (>7h) — risco de leads sem atendimento (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar em maturação).
...
## Resumo do tick (18:01)
- **Pipeline (snapshot 18:01)**: 170 leads · **12 respondidos** · 27 sequência · **104 novos** · 25 bounce (CSV) / 41 cache · 2 encerrados · **ativos: 131** — inalterado (contagem direta; caixa sem verificação desde 11:01).
...
- **Vulnerabilidade exposta**: todo o pipeline de email depende de UMA senha de app — quando cai, sync + check-replies + envio caem juntos. Watchdog com teste SMTP/IMAP alarma no 1º tick da queda — funcionou (14 ticks consecutivos de alerta sem falha de detecção).

## Próximos passos recomendados
1. **URGENTE**: regenerar senha de app e atualizar `bot/config.json` → rodar sync + check-replies (há >7h de caixa não verificada desde 11:01).
