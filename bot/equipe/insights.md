# Insights — Master Óleo · 09/09/2026 (tick 14:02)

## 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (6º tick consecutivo)
- **Estado**: credencial de `masteroleo.eco@gmail.com` (bot/config.json) **continua rejeitada** — `AUTHENTICATIONFAILED` IMAP / `535 BadCredentials` SMTP, reconfirmado neste tick (14:02, testado com e sem espaços). Revogada entre 11:01 e 11:33 de 09/09.
- **Impacto**: `sync_formspree.py` e `check-replies` **FALHARAM** pelo 6º tick. **Caixa NÃO verificada desde 11:01** — risco de leads sem atendimento (Selmi, Savegnago, Grupo IMC, Tauste, Enxuto, Chimar em maturação).
...
- **Pipeline (snapshot 14:02)**: 170 leads · **12 respondidos** · 27 sequência · **104 novos** · 25 bounce (CSV) / 41 cache · 2 encerrados · **ativos: 131** — inalterado (contagem direta).
