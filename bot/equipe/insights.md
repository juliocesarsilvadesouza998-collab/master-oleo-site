# Insights — Master Óleo · 10/09/2026 (tick 14:02)

## 🚨 ALERTA CRÍTICO MANTIDO — senha de app do Gmail REVOGADA (26º tick consecutivo)

- **Estado**: credencial de `masteroleo.eco@gmail.com` (bot/config.json) **continua rejeitada** — `AUTHENTICATIONFAILED` IMAP / `535 BadCredentials` SMTP, reconfirmado neste tick (14:02 de 10/09, testado com e sem espaços — senha de 16 chars no formato do Gmail). Revogada entre 11:01 e 11:33 de 09/09.

- **Impacto**: `sync_formspree.py`, `check-replies` e **qualquer envio** **FALHARAM** pelo 26º tick. **Caixa NÃO verificada desde 11:01 de 09/09 (~27h)** — respostas de Savegnago, Selmi, Grupo IMC, Tauste, Enxuto, Chimar podem estar órfãs na INBOX.
...
- `prospecao_followup.py` **tentou os 7 novamente e falhou no SMTP (535)** — nenhum enviado, nenhum marcado (`fp1_em` vazio confirmado no CSV). **Retry automático assim que a credencial voltar** — sem ação manual necessária além da senha.

- ⏳ **Atenção**: se a senha não for regenerada, o **FP2 desses leads vence ~13/09 (dia 6)** — o atraso vai dobrar.

## Resumo do tick (14:02)

- **Pipeline (snapshot 14:02)**: 170 leads · **12 respondidos** · 27 sequência · **104 novos** · 25 bounce (CSV) / 41 cache · 2 encerrados · **ativos: 131** — **inalterado** (contagem direta no leads.csv).

- **Ação do tick**: rotina — sync ❌ (credencial, 26º), corrigir_emails ✅ offline (0 pendentes), watchdog ⚠️ exit 1 (credencial + 7 atrasados), followups ❌ 7×535, send_sequence ❌ (535, nada enviado, sem corromper estado), check-replies ❌ (caixa cega), enviar_lote --status ✅ (0 pendentes).
...
- **Fila**: 131/120/28/0 pendentes. **Relatório ESG**: nenhum pedido detectável (caixa inacessível). **Emails no tick**: 0. **Dashboard**: 145 apresentações / 122 entregues / 23 bounces (16%) / 11 respostas / taxa 7,6%.
